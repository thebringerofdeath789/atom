...
public final class ServiceManager {
    ...
    public ServiceManager(Iterable<? extends Service> iterable) {
        ...
        if (copyOf.isEmpty()) {
            ...
            copyOf = ImmutableList.m1542of(new NoOpService());
        }
        ...
        void awaitHealthy(long j, TimeUnit timeUnit) throws TimeoutException {
            ...
            try {
                if (!this.monitor.waitForUninterruptibly(this.awaitHealthGuard, j, timeUnit)) {
                    throw new TimeoutException("Timeout waiting for the services to become healthy. The following services have not started: " + Multimaps.filterKeys((SetMultimap) this.servicesByState, Predicates.m1520in(ImmutableSet.m1585of(Service.State.NEW, Service.State.STARTING))));
                }
                checkHealthy();
            } finally {
                this.monitor.leave();
        ...
        void awaitStopped(long j, TimeUnit timeUnit) throws TimeoutException {
            ...
            try {
                if (this.monitor.waitForUninterruptibly(this.stoppedGuard, j, timeUnit)) {
                    ...
                    throw new TimeoutException("Timeout waiting for the services to stop. The following services have not stopped: " + Multimaps.filterKeys((SetMultimap) this.servicesByState, Predicates.not(Predicates.m1520in(EnumSet.of(Service.State.TERMINATED, Service.State.FAILED)))));
                }
            } finally {
                this.monitor.leave();
            }
