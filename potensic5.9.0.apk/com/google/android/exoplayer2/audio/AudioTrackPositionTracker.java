...
final class AudioTrackPositionTracker {
    ...
    public void setAudioTrack(AudioTrack audioTrack, boolean z, int i, int i2, int i3) {
        ...
        this.hasData = false;
        this.stopTimestampUs = C0858C.TIME_UNSET;
        this.forceResetWorkaroundTimeMs = C0858C.TIME_UNSET;
    ...
    public long getCurrentPositionUs(boolean z) {
        ...
        if (!this.notifiedPositionIncreasing) {
            ...
            if (j > j4) {
                ...
                this.listener.onPositionAdvancing(System.currentTimeMillis() - C0858C.usToMs(Util.getPlayoutDurationForMediaDuration(C0858C.usToMs(j - j4), this.audioTrackPlaybackSpeed)));
            }
        }
    ...
    public boolean mayHandleBuffer(long j) {
        ...
        if (z && !hasPendingData && playState != 1) {
            this.listener.onUnderrun(this.bufferSize, C0858C.usToMs(this.bufferSizeUs));
        }
        return true;
    }
    ...
    public long getPendingBufferDurationMs(long j) {
        return C0858C.usToMs(framesToDurationUs(j - getPlaybackHeadPosition()));
    }

    public boolean isStalled(long j) {
        return this.forceResetWorkaroundTimeMs != C0858C.TIME_UNSET && j > 0 && SystemClock.elapsedRealtime() - this.forceResetWorkaroundTimeMs >= FORCE_RESET_WORKAROUND_TIMEOUT_MS;
    }
    ...
    public boolean pause() {
        resetSyncParams();
        if (this.stopTimestampUs != C0858C.TIME_UNSET) {
            return false;
        }
        ((AudioTimestampPoller) Assertions.checkNotNull(this.audioTimestampPoller)).reset();
        return true;
    ...
    private long getPlaybackHeadPosition() {
        ...
        if (Util.SDK_INT <= 29) {
            if (playbackHeadPosition == 0 && this.lastRawPlaybackHeadPosition > 0 && playState == 3) {
                if (this.forceResetWorkaroundTimeMs == C0858C.TIME_UNSET) {
