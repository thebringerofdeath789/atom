...
public final class Tx3gDecoder extends SimpleSubtitleDecoder {
    ...
    public Tx3gDecoder(List<byte[]> list) {
        ...
        if (list.size() == 1 && (list.get(0).length == 48 || list.get(0).length == 53)) {
            ...
            this.defaultFontFamily = TX3G_SERIF.equals(Util.fromUtf8Bytes(bArr, 43, bArr.length - 43)) ? C0858C.SERIF_NAME : "sans-serif";
            int i = bArr[25] * 20;
    ...
    private void applyStyleRecord(ParsableByteArray parsableByteArray, SpannableStringBuilder spannableStringBuilder) throws SubtitleDecoderException {
        ...
        if (readUnsignedShort2 > spannableStringBuilder.length()) {
            Log.m1501w(TAG, new StringBuilder(68).append("Truncating styl end (").append(readUnsignedShort2).append(") to cueText.length() (").append(spannableStringBuilder.length()).append(").").toString());
            i = spannableStringBuilder.length();
        } else {
            i = readUnsignedShort2;
        }
        if (readUnsignedShort >= i) {
            Log.m1501w(TAG, new StringBuilder(60).append("Ignoring styl with start (").append(readUnsignedShort).append(") >= end (").append(i).append(").").toString());
            return;
        }
        int i2 = i;
