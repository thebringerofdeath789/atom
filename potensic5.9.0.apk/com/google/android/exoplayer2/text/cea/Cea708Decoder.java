...
public final class Cea708Decoder extends CeaDecoder {
            ...
            if (i == 2 || i == 3) {
                if (z) {
                    if (i == 3) {
                        ...
                        if (i3 != -1 && i2 != (i3 + 1) % 4) {
                            resetCueBuilders();
                            Log.m1501w(TAG, new StringBuilder(71).append("Sequence number discontinuity. previous=").append(this.previousSequenceNumber).append(" current=").append(i2).toString());
                        }
                        ...
                        if (dtvCcPacket3 == null) {
                            Log.m1497e(TAG, "Encountered DTVCC_PACKET_DATA before DTVCC_PACKET_START");
                        } else {
    ...
    private void processCurrentPacket() {
        if (this.currentDtvCcPacket.currentIndex != (this.currentDtvCcPacket.packetSize * 2) - 1) {
            ...
            Log.m1495d(TAG, new StringBuilder(115).append("DtvCcPacket ended prematurely; size is ").append(i).append(", but current index is ").append(this.currentDtvCcPacket.currentIndex).append(" (sequence number ").append(this.currentDtvCcPacket.sequenceNumber).append(");").toString());
        }
        ...
        if (readBits == 7) {
            ...
            if (readBits < 7) {
                Log.m1501w(TAG, new StringBuilder(44).append("Invalid extended service number: ").append(readBits).toString());
            }
        }
        if (readBits2 == 0) {
            if (readBits != 0) {
                Log.m1501w(TAG, new StringBuilder(59).append("serviceNumber is non-zero (").append(readBits).append(") when blockSize is 0").toString());
                return;
            }
            return;
        }
            ...
            if (readBits3 == 16) {
                ...
                if (readBits4 <= 31) {
                    ...
                    if (readBits4 <= 127) {
                        ...
                        Log.m1501w(TAG, new StringBuilder(37).append("Invalid extended command: ").append(readBits4).toString());
                    }
                    z = true;
                }
                ...
                if (readBits3 <= 127) {
                    ...
                    Log.m1501w(TAG, new StringBuilder(33).append("Invalid base command: ").append(readBits3).toString());
                }
                z = true;
            }
        }
    ...
    private void handleC0Command(int i) {
        if (i != 0) {
            ...
            switch (i) {
                ...
                default:
                    if (i >= 17 && i <= 23) {
                        Log.m1501w(TAG, new StringBuilder(55).append("Currently unsupported COMMAND_EXT1 Command: ").append(i).toString());
                        this.serviceBlockPacket.skipBits(8);
                        break;
                    } else if (i >= 24 && i <= 31) {
                        Log.m1501w(TAG, new StringBuilder(54).append("Currently unsupported COMMAND_P16 Command: ").append(i).toString());
                        this.serviceBlockPacket.skipBits(16);
                        break;
                    } else {
                        Log.m1501w(TAG, new StringBuilder(31).append("Invalid C0 command: ").append(i).toString());
                        break;
                    }
                    break;
            }
    ...
    private void handleC1Command(int i) {
        ...
        switch (i) {
            ...
            case 150:
            default:
                Log.m1501w(TAG, new StringBuilder(31).append("Invalid C1 command: ").append(i).toString());
                break;
            case 151:
                if (!this.currentCueInfoBuilder.isDefined()) {
    ...
    private void handleG2Character(int i) {
        ...
        if (i == 57) {
            this.currentCueInfoBuilder.append(Typography.f4068tm);
            return;
        }
        if (i == 58) {
                ...
                default:
                    switch (i) {
                        ...
                        case 127:
                            ...
                            break;
                        default:
                            Log.m1501w(TAG, new StringBuilder(33).append("Invalid G2 character: ").append(i).toString());
                            break;
                    }
            }
            return;
    ...
    private void handleG3Character(int i) {
        if (i == 160) {
            ...
            Log.m1501w(TAG, new StringBuilder(33).append("Invalid G3 character: ").append(i).toString());
            this.currentCueInfoBuilder.append(NameUtil.USCORE);
        }
    }
    ...
    private static final class Cea708CueInfo {
        private static final Comparator<Cea708CueInfo> LEAST_IMPORTANT_FIRST = new Comparator() { // from class: com.google.android.exoplayer2.text.cea.-$$Lambda$Cea708Decoder$Cea708CueInfo$Pxmft5kU-whF6SchrLmcyiHvOx8
