...
public final class FrameworkMediaDrm implements ExoMediaDrm {
    ...
    static /* synthetic */ ExoMediaDrm lambda$static$0(UUID uuid) {
        try {
            ...
            Log.m1497e(TAG, new StringBuilder(String.valueOf(valueOf).length() + 53).append("Failed to instantiate a FrameworkMediaDrm for uuid: ").append(valueOf).append(".").toString());
    ...
    private FrameworkMediaDrm(UUID uuid) throws UnsupportedSchemeException {
        ...
        if (C0858C.WIDEVINE_UUID.equals(uuid) && needsForceWidevineL3Workaround()) {
    ...
    public byte[] provideKeyResponse(byte[] bArr, byte[] bArr2) throws NotProvisionedException, DeniedByServerException {
        if (C0858C.CLEARKEY_UUID.equals(this.uuid)) {
    ...
    public FrameworkMediaCrypto createMediaCrypto(byte[] bArr) throws MediaCryptoException {
        return new FrameworkMediaCrypto(adjustUuid(this.uuid), bArr, Util.SDK_INT < 21 && C0858C.WIDEVINE_UUID.equals(this.uuid) && "L3".equals(getPropertyString("securityLevel")));
    }
    ...
    private static DrmInitData.SchemeData getSchemeData(UUID uuid, List<DrmInitData.SchemeData> list) {
        boolean z;
        if (!C0858C.WIDEVINE_UUID.equals(uuid)) {
            return list.get(0);
        }
    ...
    private static UUID adjustUuid(UUID uuid) {
        return (Util.SDK_INT >= 27 || !C0858C.CLEARKEY_UUID.equals(uuid)) ? uuid : C0858C.COMMON_PSSH_UUID;
    }

    private static byte[] adjustRequestInitData(UUID uuid, byte[] bArr) {
        byte[] parseSchemeSpecificData;
        if (C0858C.PLAYREADY_UUID.equals(uuid)) {
            ...
            bArr = PsshAtomUtil.buildPsshAtom(C0858C.PLAYREADY_UUID, addLaUrlAttributeIfMissing(bArr));
        }
        return (((Util.SDK_INT >= 23 || !C0858C.WIDEVINE_UUID.equals(uuid)) && !(C0858C.PLAYREADY_UUID.equals(uuid) && "Amazon".equals(Util.MANUFACTURER) && ("AFTB".equals(Util.MODEL) || "AFTS".equals(Util.MODEL) || "AFTM".equals(Util.MODEL) || "AFTT".equals(Util.MODEL)))) || (parseSchemeSpecificData = PsshAtomUtil.parseSchemeSpecificData(bArr, uuid)) == null) ? bArr : parseSchemeSpecificData;
    }

    private static String adjustRequestMimeType(UUID uuid, String str) {
        return (Util.SDK_INT < 26 && C0858C.CLEARKEY_UUID.equals(uuid) && (MimeTypes.VIDEO_MP4.equals(str) || MimeTypes.AUDIO_MP4.equals(str))) ? "cenc" : str;
    }

    private static byte[] adjustRequestData(UUID uuid, byte[] bArr) {
        return C0858C.CLEARKEY_UUID.equals(uuid) ? ClearKeyUtil.adjustRequestData(bArr) : bArr;
    }
    ...
    private static byte[] addLaUrlAttributeIfMissing(byte[] bArr) {
        ...
        if (readLittleEndianShort != 1 || readLittleEndianShort2 != 1) {
            Log.m1499i(TAG, "Unexpected record count or type. Skipping LA_URL workaround.");
            return bArr;
        }
        ...
        if (indexOf == -1) {
            Log.m1501w(TAG, "Could not find the </DATA> tag. Skipping LA_URL workaround.");
        }
