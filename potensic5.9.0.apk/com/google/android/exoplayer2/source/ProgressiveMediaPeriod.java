...
final class ProgressiveMediaPeriod implements MediaPeriod, ExtractorOutput, Loader.Callback<ExtractingLoadable>, Loader.ReleaseCallback, SampleQueue.UpstreamFormatChangedListener {
    ...
    private long pendingResetPositionUs = C0858C.TIME_UNSET;
    ...
    public void onLoadCompleted(ExtractingLoadable extractingLoadable, long j, long j2) {
        SeekMap seekMap;
        if (this.durationUs == C0858C.TIME_UNSET && (seekMap = this.seekMap) != null) {
    ...
    public Loader.LoadErrorAction onLoadError(ExtractingLoadable extractingLoadable, long j, long j2, IOException iOException, int i) {
        ...
        StatsDataSource statsDataSource = extractingLoadable.dataSource;
        LoadEventInfo loadEventInfo = new LoadEventInfo(extractingLoadable.loadTaskId, extractingLoadable.dataSpec, statsDataSource.getLastOpenedUri(), statsDataSource.getLastResponseHeaders(), j, j2, statsDataSource.getBytesRead());
        long retryDelayMsFor = this.loadErrorHandlingPolicy.getRetryDelayMsFor(new LoadErrorHandlingPolicy.LoadErrorInfo(loadEventInfo, new MediaLoadData(1, -1, null, 0, null, C0858C.usToMs(extractingLoadable.seekTimeUs), C0858C.usToMs(this.durationUs)), iOException, i));
    ...
    private void lambda$seekMap$1$ProgressiveMediaPeriod(SeekMap seekMap) {
        this.seekMap = this.icyHeaders == null ? seekMap : new SeekMap.Unseekable(C0858C.TIME_UNSET);
        this.durationUs = seekMap.getDurationUs();
        boolean z = this.length == -1 && seekMap.getDurationUs() == C0858C.TIME_UNSET;
        this.isLive = z;
        this.dataType = z ? 7 : 1;
    ...
    private void startLoading() {
        ...
        if (this.prepared) {
            ...
            if (j != C0858C.TIME_UNSET && this.pendingResetPositionUs > j) {
                this.loadingFinished = true;
    ...
    private boolean configureRetry(ExtractingLoadable extractingLoadable, int i) {
        SeekMap seekMap;
        if (this.length != -1 || ((seekMap = this.seekMap) != null && seekMap.getDurationUs() != C0858C.TIME_UNSET)) {
    ...
    private static final class TrackId {
        ...
        public boolean equals(Object obj) {
            ...
            return this.f1900id == trackId.f1900id && this.isIcyTrack == trackId.isIcyTrack;
        }

        public int hashCode() {
            return (this.f1900id * 31) + (this.isIcyTrack ? 1 : 0);
        }
    }
