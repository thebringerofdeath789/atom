...
public class DashManifest implements FilterableManifest<DashManifest> {
    ...
    public final long getPeriodDurationMs(int i) {
        if (i == this.periods.size() - 1) {
            ...
            return j == C0858C.TIME_UNSET ? C0858C.TIME_UNSET : j - this.periods.get(i).startMs;
        }
        return this.periods.get(i + 1).startMs - this.periods.get(i).startMs;
    }

    public final long getPeriodDurationUs(int i) {
        return C0858C.msToUs(getPeriodDurationMs(i));
    }
    ...
    public final DashManifest copy(List<StreamKey> list) {
        ...
        while (true) {
            ...
            if (((StreamKey) linkedList.peek()).periodIndex != i) {
                ...
                if (periodDurationMs != C0858C.TIME_UNSET) {
                    j2 += periodDurationMs;
                }
            } else {
                Period period = getPeriod(i);
                arrayList.add(new Period(period.f1906id, period.startMs - j2, copyAdaptationSets(period.adaptationSets, linkedList), period.eventStreams));
            }
            i++;
        }
    ...
    private static ArrayList<AdaptationSet> copyAdaptationSets(List<AdaptationSet> list, LinkedList<StreamKey> linkedList) {
        ...
        do {
            ...
            arrayList.add(new AdaptationSet(adaptationSet.f1904id, adaptationSet.type, arrayList2, adaptationSet.accessibilityDescriptors, adaptationSet.essentialProperties, adaptationSet.supplementalProperties));
