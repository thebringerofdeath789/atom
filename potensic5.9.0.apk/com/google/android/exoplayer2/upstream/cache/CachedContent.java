...
final class CachedContent {
    ...
    public SimpleCacheSpan setLastTouchTimestamp(SimpleCacheSpan simpleCacheSpan, long j, boolean z) {
        ...
        if (z) {
            File cacheFile = SimpleCacheSpan.getCacheFile((File) Assertions.checkNotNull(file.getParentFile()), this.f1915id, simpleCacheSpan.position, j);
            if (file.renameTo(cacheFile)) {
                file = cacheFile;
            } else {
                String valueOf = String.valueOf(file);
                String valueOf2 = String.valueOf(cacheFile);
                Log.m1501w(TAG, new StringBuilder(String.valueOf(valueOf).length() + 21 + String.valueOf(valueOf2).length()).append("Failed to rename ").append(valueOf).append(" to ").append(valueOf2).toString());
            }
        }
    ...
    public int hashCode() {
        return (((this.f1915id * 31) + this.key.hashCode()) * 31) + this.metadata.hashCode();
    }
    ...
    public boolean equals(Object obj) {
        ...
        return this.f1915id == cachedContent.f1915id && this.key.equals(cachedContent.key) && this.cachedSpans.equals(cachedContent.cachedSpans) && this.metadata.equals(cachedContent.metadata);
    }
