    ...
    public boolean dispatchPrevious(Player player) {
        ...
        if (!currentTimeline.isEmpty() && !player.isPlayingAd()) {
            ...
            if (previousWindowIndex != -1 && (player.getCurrentPosition() <= 3000 || z)) {
                player.seekTo(previousWindowIndex, C0858C.TIME_UNSET);
            } else if (!z) {
    ...
    public boolean dispatchNext(Player player) {
        ...
        if (!currentTimeline.isEmpty() && !player.isPlayingAd()) {
            ...
            if (nextWindowIndex != -1) {
                player.seekTo(nextWindowIndex, C0858C.TIME_UNSET);
            } else if (this.window.isLive() && this.window.isDynamic) {
                player.seekTo(currentWindowIndex, C0858C.TIME_UNSET);
            }
        }
        return true;
    }
