...
public final class PesReader implements TsPayloadReader {
    ...
    public final void consume(ParsableByteArray parsableByteArray, int i) throws ParserException {
        ...
        if ((i & 1) != 0) {
            ...
            if (i2 != 0 && i2 != 1) {
                if (i2 == 2) {
                    Log.m1501w(TAG, "Unexpected start indicator reading extended header");
                } else if (i2 == 3) {
                    int i3 = this.payloadSize;
                    if (i3 != -1) {
                        Log.m1501w(TAG, new StringBuilder(59).append("Unexpected start indicator: expected ").append(i3).append(" more bytes").toString());
                    }
    ...
    private boolean parseHeader() {
        ...
        if (readBits != 1) {
            Log.m1501w(TAG, new StringBuilder(41).append("Unexpected start code prefix: ").append(readBits).toString());
            this.payloadSize = -1;
            return false;
        }
        ...
        if (readBits2 == 0) {
            ...
            this.payloadSize = i;
            if (i < 0) {
                Log.m1501w(TAG, new StringBuilder(47).append("Found negative packet payload size: ").append(i).toString());
                this.payloadSize = -1;
            }
        }
        return true;
