...
public final class PsshAtomUtil {
    ...
    public static byte[] parseSchemeSpecificData(byte[] bArr, UUID uuid) {
        ...
        if (!uuid.equals(parsePsshAtom.uuid)) {
            ...
            Log.m1501w(TAG, new StringBuilder(String.valueOf(valueOf).length() + 33 + String.valueOf(valueOf2).length()).append("UUID mismatch. Expected: ").append(valueOf).append(", got: ").append(valueOf2).append(".").toString());
            return null;
        }
    ...
    private static PsshAtom parsePsshAtom(byte[] bArr) {
        ...
        if (parseFullAtomVersion > 1) {
            Log.m1501w(TAG, new StringBuilder(37).append("Unsupported pssh version: ").append(parseFullAtomVersion).toString());
            return null;
        }
