...
public class RemoteFileThumbLoader {
    ...
    private void download(RemoteFile remoteFile, OnFileCallback onFileCallback) {
        ...
        DDLog.m1684e("图库创建 thumbnailFileReceiver  :" + this.thumbnailFileReceiver);
    ...
    public void downloadNextThumb() throws Exception {
        if (UsbGalleryManager.getInstance().isEnterGallery()) {
            if (this.list.get(this.downloadIndex).getThumbnailUrl() != null) {
                DDLog.m1684e("缩略图已存在 ：" + this.list.get(this.downloadIndex).getThumbnailUrl());
            ...
            DDLog.m1684e("图库缩略图：" + remoteFile.getFileName());
            download(remoteFile, new OnFileCallback() { // from class: com.logan.usb.gallery.RemoteFileThumbLoader.1
                ...
                public void onFile(File file, boolean z) {
                    try {
                        if (z) {
                            DDLog.m1684e("图库缩略图 损坏：" + remoteFile.getFileName());
                        } else {
                            DDLog.m1684e("图库缩略图 完整：" + remoteFile.getFileName());
                        }
                        PhoneConfig.mainHandler.removeCallbacks(RemoteFileThumbLoader.this.fileThumbRunnable);
                        if (z) {
                        ...
                        DDLog.m1684e("图库下载缩略图报错:" + e2.getMessage());
                        RemoteFileThumbLoader.this.onAllDone();
                    }
                }
                ...
                public void onFailed() {
                    try {
                        DDLog.m1684e("图库缩略图 下载失败：" + remoteFile.getFileName());
                        RemoteFileThumbLoader.this.downloadNextThumb();
                    } catch (Exception e) {
                        DDLog.m1684e("图库加载缩略图失败:" + e);
                    }
    ...
    public void onReceived(byte[] bArr) {
        ...
        if (bArr[payloadIndex + 1] != 0) {
            if (this.fileCallback != null) {
                try {
                    ...
                    if (!file3.exists()) {
                        ((BitmapDrawable) ResourcesCompat.getDrawable(PhoneConfig.applicationContext.getResources(), C2962R.mipmap.img_bg_thumbnail_broken, null)).getBitmap().compress(Bitmap.CompressFormat.PNG, 100, new FileOutputStream(file3.getAbsolutePath()));
                    }
        ...
        if (!(bArr[payloadIndex + 2] == 2)) {
            ...
            if (bArr.length > i5 + unsignedShortFromByteArr) {
                try {
                    ...
                    DDLog.m1684e("图库接收数据出错1" + e.getMessage());
                    return;
                }
            }
            return;
        ...
        DDLog.m1684e("图库最后一帧长度:" + unsignedShortFromByteArr2);
        ...
        if (bArr.length > i6 + unsignedShortFromByteArr2) {
            try {
                ...
                DDLog.m1684e("P1PRO 图库接收数据出错2:" + e2.getMessage());
            }
        }
        FileReceiver fileReceiver = this.thumbnailFileReceiver;
        if (fileReceiver != null) {
            fileReceiver.finish();
            DDLog.m1684e("图库缩略图接收完成");
            byte[] fileMD5 = Md5Util.getFileMD5(this.thumbnailFileReceiver.getFile());
            DDLog.m1684e("本地图库 MD5:" + ParseUtil.byteToHexString(fileMD5));
            if (fileMD5 != null) {
                byte[] bArr3 = new byte[32];
                System.arraycopy(bArr, i4, bArr3, 0, 32);
                DDLog.m1684e("远程图库 MD5:" + ParseUtil.byteToHexString(bArr3));
