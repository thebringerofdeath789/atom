...
public class AsyncUsbParser extends Thread {
    private OnFrameOutputListener frameOutputListener;
    private final int USB_FRAME_HEAD_LEN = 16;
    private final ConcurrentLinkedQueue<UsbBytes> usbDataQueue = new ConcurrentLinkedQueue<>();
    ...
    private FileOutputStream testData = null;
    private final byte[] HEAD = ParseUtil.concatAll(new byte[]{-1, -1}, "HeAd".getBytes(), new byte[]{-1, -1});
    ...
    public synchronized void put(UsbBytes usbBytes) {
        if (this.isRunning.get()) {
            if (this.usbDataQueue.size() > 100) {
                this.usbDataQueue.clear();
                DDLog.m1684e("USB数据解析压力大 已清空缓存!!");
            }
            this.usbDataQueue.offer(usbBytes);
        }
    }
    ...
    public void run() {
        ...
        while (this.isRunning.get()) {
            try {
                if (this.usbDataQueue.isEmpty()) {
                    Thread.sleep(1L);
                }
                while (this.isRunning.get() && !this.usbDataQueue.isEmpty()) {
                    UsbBytes poll = this.usbDataQueue.poll();
                    if (poll != null) {
                        parseUsbFrame(poll);
                    }
                }
            } catch (Exception e) {
                DDLog.m1684e("解析出错1:" + e);
            }
        }
        this.isRunning.set(false);
        FileOutputStream fileOutputStream = this.testData;
    ...
    private synchronized void parseUsbFrame(UsbBytes usbBytes) throws Exception {
    ...
    private void parseBuffer(Buffer buffer, byte b) {
        ...
        if (buffer.getWriteIndex() > 6) {
            int i2 = -1;
            int i3 = 0;
            while (i3 < buffer.getWriteIndex() - 6 && this.isRunning.get()) {
