            ...
            public void run() {
                ...
                if (!CustomClient.this.isConnected.get()) {
                    try {
                        ...
                        DDLog.m1691w("socket连接失败:" + e.getMessage());
                        CustomClient.this.close(null);
                    }
                }
            ...
            public void run() {
                synchronized (CustomClient.this.sendLock) {
                    try {
                        ...
                        CustomClient.this.buffer.clear();
                        DDLog.m1691w("socket发送线程结束");
                    } catch (Exception e) {
                        DDLog.m1684e("socket发送错误:" + e.getMessage());
                        CustomClient.this.close(null);
                    }
                }
            }
            ...
            public void run() {
                ...
                try {
                    ...
                    while (CustomClient.this.reader != null && CustomClient.this.isConnected.get() && (read = CustomClient.this.reader.read(CustomClient.this.result)) != -1) {
                        ...
                        DDLog.m1691w("收到数据:" + ParseUtil.byteToHexString(CustomClient.this.result, read));
                    }
                    DDLog.m1691w("socket接收线程结束");
                } catch (Exception e) {
                    DDLog.m1684e("socket断开 " + e.getMessage());
                    CustomClient.this.close(null);
                }
            }
        });
