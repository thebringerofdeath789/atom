...
public class MagCalibrationResult {
    ...
    public boolean isHorizontalCalibration() {
        return (this.f2483x != 0 || isCalibrationFinish() || isCalibrationTimeout() || isCalibrationFailed()) ? false : true;
    }

    public boolean isVerticalCalibration() {
        return (this.f2483x != 1 || isCalibrationFinish() || isCalibrationTimeout() || isCalibrationFailed()) ? false : true;
    }

    public boolean isCalibrationTimeout() {
        return this.f2484y == 1;
    }

    public boolean isCalibrationFinish() {
        return this.f2484y == 2;
    }

    public boolean isCalibrationFailed() {
        return this.f2484y == 3;
    }

    public String toString() {
        if (isCalibrationFailed()) {
            return "MagCalibrationResult{水平校准=" + isHorizontalCalibration() + ", 垂直校准=" + isVerticalCalibration() + ", 校准超时=" + isCalibrationTimeout() + ", 校准失败=" + isCalibrationFailed() + ", 校准成功=" + isCalibrationFinish() + ", scale_result=" + Arrays.toString(this.scale_result) + ", bias_result=" + Arrays.toString(this.bias_result) + ", offdiag_result=" + Arrays.toString(this.offdiag_result) + ", reference_flag_result=" + Arrays.toString(this.reference_flag_result) + ", mg_db_result=" + Arrays.toString(this.mg_db_result) + ", x = " + this.f2483x + ", y = " + this.f2484y + '}';
        }
        if (isCalibrationTimeout()) {
            return "MagCalibrationResult{水平校准=" + isHorizontalCalibration() + ", 垂直校准=" + isVerticalCalibration() + ", 校准超时=" + isCalibrationTimeout() + ", 校准成功=" + isCalibrationFinish() + ", x = " + this.f2483x + ", y = " + this.f2484y + '}';
        }
