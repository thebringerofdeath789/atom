...
public class UdpClient {
                ...
                while (UdpClient.this.isConnected) {
                    try {
                        ...
                        if (datagramPacket.getData() == null) {
                            DDLog.m1691w("没有udp数据");
                        } else {
                            int length = datagramPacket.getLength();
                            DDLog.m1691w("udp 接收数据:" + ParseUtil.byteToHexString(UdpClient.this.result, length));
                            DataParser.getInstance().write(UdpClient.this.result, length);
                        }
                    } catch (Exception e) {
                        DDLog.m1684e("接收udp数据失败:" + e.getMessage());
                    }
                }
            }
        });
    ...
    public void send(byte[] bArr) {
        synchronized (this.lock) {
            DDLog.m1691w("发送udp数据:" + ParseUtil.byteToHexString(bArr));
