...
public class UpgradeManager {
            ...
            public void run() {
                try {
                    ...
                    while (!UpgradeManager.this.isUpgradeShakeHandSuccess) {
                        ...
                        DDLog.m1684e("发送升级握手：" + ParseUtil.byteToHexString(wrap));
                        DDLog.m1684e("发送升级握手：" + ParseUtil.byteToHexString(new byte[]{UpgradeManager.this.upgradeFuncCode}));
        ...
        this.threadPool.execute(new Runnable() { // from class: com.logan.upgrade.local.flight.UpgradeManager.2
            ...
            public void run() {
                ...
                if (file.exists()) {
                    ...
                    DDLog.m1684e("升级文件:len=" + UpgradeManager.this.fileGiver.getFileLength() + ",checkCode:" + UpgradeManager.this.fileGiver.getCheckCode());
                ...
                DDLog.m1684e("上传升级文件失败:文件不存在:" + str);
            }
        });
    }
        ...
        this.threadPool.execute(new Runnable() { // from class: com.logan.upgrade.local.flight.UpgradeManager.3
            ...
            */
            public void run() {
                ...
                */
                throw new UnsupportedOperationException("Method not decompiled: com.logan.upgrade.local.flight.UpgradeManager.RunnableC30093.run():void");
            }
        });
    }
    ...
    public void onRevData(int i, byte[] bArr, int i2) {
        if (i == -15) {
            DDLog.m1684e("【升级握手】: " + ParseUtil.byteToHexString(bArr));
            FlightRevData.get().getRevShakeHandData().parse(bArr, i2);
            DDLog.m1684e("收到升级握手:" + this.isUpgradeShakeHandSuccess);
        ...
        if (i == -11) {
            DDLog.m1684e("【升级长度告知】: " + ParseUtil.byteToHexString(bArr));
        ...
        if (i == -9) {
            DDLog.m1684e("【升级上传文件】: " + ParseUtil.byteToHexString(bArr));
                ...
                try {
                    if (this.sendFile != null && revFwFileData.getPackageIndex() == this.sendFile.getIndex()) {
                        ...
                        if (!revFwFileData.isSaveSuccess()) {
                            DDLog.m1684e("上传第" + ((int) this.sendFile.getIndex()) + InternalZipConstants.ZIP_FILE_SEPARATOR + this.totalNum + "包升级文件：" + revFwFileData.isSaveSuccess() + "," + revFwFileData.isNotSave());
                        } else {
                            DDLog.m1684e("上传第" + ((int) this.sendFile.getIndex()) + InternalZipConstants.ZIP_FILE_SEPARATOR + this.totalNum + "包升级文件：" + revFwFileData.isSaveSuccess());
                        }
                    }
                } catch (Exception e) {
                    DDLog.m1684e("升级上传文件出错 e:" + e);
                }
            }
            return;
        }
        ...
        DDLog.m1684e("【升级运行固件】: " + ParseUtil.byteToHexString(bArr));
    ...
    public void onReceiveFlightVersion(FlightRevVersionData flightRevVersionData) {
        DDLog.m1687i("升级成功 接收飞机数据");
        upgradeSuccess();
        if (UsbConfig.isUsbConnected) {
            DDLog.m1691w("升级完成打开遥控器数据");
