...
public class UdpClient {
    ...
    /* renamed from: IP */
    private final String f2492IP = "192.168.42.30";
    ...
    static /* synthetic */ int access$712(UdpClient udpClient, int i) {
        int i2 = udpClient.totalReadLen + i;
        udpClient.totalReadLen = i2;
        return i2;
    }
        ...
        this.threadPool.execute(new Runnable() { // from class: com.logan.h264.UdpClient.1
            ...
            public void run() {
                try {
                    ...
                    DDLog.m1691w("连接错误:" + e.getMessage());
                    ...
                    try {
                        ...
                        if (UdpClient.this.receivePacket.getData() != null) {
                            ...
                            if (copyOf.length <= 5 || copyOf[0] != 0 || copyOf[1] != 0 || copyOf[2] != 1 || (copyOf[3] != -96 && copyOf[3] != -95)) {
                                ...
                                UdpClient.access$712(UdpClient.this, length);
                            } else {
                        ...
                        DDLog.m1691w("读取错误:" + e.getMessage());
                    }
                }
            }
        });
    ...
    public void sendData(final byte[] bArr) {
        DDLog.m1691w("发送握手 isconnect:" + this.isConnected);
        if (this.isConnected) {
                ...
                public void run() {
                    try {
                        synchronized (UdpClient.this.lock) {
                            ...
                            DDLog.m1691w("数据已发送:" + ParseUtil.byteToHexString(bArr, 20));
                        }
                    } catch (Exception e) {
                        DDLog.m1691w("发送错误:" + e.getMessage());
                    }
                }
            });
        }
