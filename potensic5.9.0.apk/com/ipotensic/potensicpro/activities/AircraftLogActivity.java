...
public class AircraftLogActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle bundle) {
        ...
        setContentView(C2640R.layout.activity_aircraft_log);
        initView();
        setToolBar();
    }

    private void initView() {
        this.ivEmptyLog = (ImageView) findViewById(C2640R.id.iv_empty_log);
        this.tvLogTips = (TextView) findViewById(C2640R.id.tv_log_tips);
        this.f2365rv = (RecyclerView) findViewById(C2640R.id.f2310rv);
        this.tvNoRecord = (TextView) findViewById(C2640R.id.tv_no_record);
        MyLinearLayoutManager myLinearLayoutManager = new MyLinearLayoutManager(this, 1, false);
        this.f2365rv.addItemDecoration(new DividerLineaItemDecoration());
        this.f2365rv.setLayoutManager(myLinearLayoutManager);
        AircraftLogAdapter aircraftLogAdapter = new AircraftLogAdapter();
        this.logAdapter = aircraftLogAdapter;
        this.f2365rv.setAdapter(aircraftLogAdapter);
    }
    ...
    private void initData() {
        ...
        new Thread(new Runnable() { // from class: com.ipotensic.potensicpro.activities.AircraftLogActivity.2
            ...
            public void run() {
                ...
                DDLog.m1684e("日志文件集合:" + files);
                if (files != null && files.length > 0) {
                    DDLog.m1684e("日志文件集合1:" + files.length);
                    ...
                    while (i < length) {
                        File file = files[i];
                        DDLog.m1684e("日志文件:" + file.getAbsolutePath());
                        ...
                        try {
                            ...
                            DDLog.m1684e("添加文件报错:" + file.getAbsolutePath());
                        }
                        i++;
                        j = lastModified;
                    }
                    for (int i2 = 0; i2 < arrayList.size(); i2++) {
                        try {
                            ...
                            DDLog.m1684e("解析日志出错:" + e.getMessage());
                        }
                    }
                    DDLog.m1684e("fileList:" + AircraftLogActivity.this.fileList.size());
                }
                try {
                    Thread.sleep(1000L);
                    ...
                    public void run() {
                        try {
                            ...
                            if (AircraftLogActivity.this.logs.size() == 0) {
                                ...
                                AircraftLogActivity.this.f2365rv.setVisibility(8);
    ...
    private void setToolBar() {
        ...
        ((TextView) toolbar.findViewById(C2640R.id.tv_code_title)).setText(getResources().getString(C2640R.string.main_aircraft_log));
    ...
    private class AircraftLogAdapter extends RecyclerView.Adapter<ViewHolder> {
        ...
        public ViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
            return new ViewHolder(LayoutInflater.from(AircraftLogActivity.this).inflate(C2640R.layout.view_adapter_aircraft_log, viewGroup, false));
        }
        ...
        public void onBindViewHolder(ViewHolder viewHolder, int i) {
            ...
            viewHolder.ivUpload.setImageResource(logBean.isSubmitted ? C2640R.mipmap.icon_aircraft_log_uploaded : C2640R.mipmap.icon_aircraft_log_upload_nor);
        ...
        public void loadLog(final LogBean logBean, final int i, String str, String str2) {
            ...
            for (int i2 = 0; i2 < list.size(); i2++) {
                ...
                try {
                    ...
                    if (file.exists()) {
                        DDLog.m1684e("upload file len:" + file.length());
                    }
                    DDLog.m1684e("upload files:" + strArr[i2]);
                } catch (Exception unused) {
                }
            }
            ...
            UserRequestPresenter.getInstance().flightLogs(PhoneConfig.usrToken, strArr, str, str2, new IFlightLogView() { // from class: com.ipotensic.potensicpro.activities.AircraftLogActivity.AircraftLogAdapter.2
                ...
                public void uploadSuccess() {
                    ToastUtil.toast(AircraftLogActivity.this, AircraftLogActivity.this.getString(C2640R.string.toast_upload_success));
                    AircraftLogActivity.this.getFiles();
                    DDLog.m1685e("flightLog", "保存成功");
                ...
                public void tokenError() {
                    DDLog.m1685e("flightLog", "token error");
                    downloadFileDialog.dismiss();
                }
                ...
                public void uploadError(Exception exc) {
                    if (exc instanceof UnknownHostException) {
                        ToastUtil.toast(AircraftLogActivity.this, PhoneConfig.applicationContext.getString(C2640R.string.toast_no_network));
                    } else {
                        ToastUtil.toast(AircraftLogActivity.this, AircraftLogActivity.this.getString(C2640R.string.toast_upload_fail));
                    }
                    DDLog.m1685e("flightLog", "uploaded error");
                    downloadFileDialog.dismiss();
                }

                @Override // com.logan.user.view.IFlightLogView
                public void fileIsExist() {
                    ToastUtil.toast(AircraftLogActivity.this, AircraftLogActivity.this.getString(C2640R.string.toast_upload_file_exist));
                    DDLog.m1685e("flightLog", "The uploaded file already exists!");
                    downloadFileDialog.dismiss();
                }

                @Override // com.logan.user.view.IFlightLogView
                public void notFrequentlyUpload() {
                    ToastUtil.toast(AircraftLogActivity.this, AircraftLogActivity.this.getString(C2640R.string.toast_upload_not_frequently));
                    DDLog.m1685e("flightLog", "Do not upload frequently!");
                    downloadFileDialog.dismiss();
                }

                @Override // com.logan.user.view.IFlightLogView
                public void missFiles() {
                    ToastUtil.toast(AircraftLogActivity.this, AircraftLogActivity.this.getString(C2640R.string.toast_upload_miss_parameters));
                    DDLog.m1685e("flightLog", "文件丢失");
                    downloadFileDialog.dismiss();
                }
                ...
                public void onProgress(float f, long j) {
                    ...
                    if (i3 != this.curProgress) {
                        DDLog.m1684e("progress:" + i3 + ",totalLen:" + j);
                        this.curProgress = i3;
                ...
                public void onError(Exception exc) {
                    DDLog.m1685e("flightLog", "上传失败:" + exc);
                }
            });
        }
        ...
        private class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
            ...
            private ViewHolder(View view) {
                ...
                this.ivUpload = (ImageView) view.findViewById(C2640R.id.iv_upload);
                view.findViewById(C2640R.id.iv_share).setOnClickListener(this);
                view.findViewById(C2640R.id.iv_upload).setOnClickListener(this);
            }
            ...
            public void onClick(View view) {
                try {
                    ...
                    if (id == C2640R.id.iv_upload) {
                        DDLog.m1684e("name:" + this.tvName.getText().toString());
                    ...
                    if (id != C2640R.id.tv_name && id == C2640R.id.iv_delete) {
                        synchronized (AircraftLogActivity.this.deleteLock) {
                            ...
                            ToastUtil.toast(AircraftLogActivity.this, AircraftLogActivity.this.getString(C2640R.string.toast_delete_success));
