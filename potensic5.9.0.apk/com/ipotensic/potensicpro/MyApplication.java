...
public class MyApplication extends MultiDexApplication {
    ...
    public void onCreate() {
        ...
        if (Objects.equals(AppUtils.getProcessName(this), AppUtils.getPackageName(this))) {
            PhoneConfig.isDownloadFromChinaMarket = false;
            ...
            initPlayer();
            try {
                Mapbox.getInstance(this, getString(C2640R.string.mapbox_access_token));
            } catch (Exception e) {
                DDLog.m1686e(TAG, "初始化地图报错", e);
            }
            ViewTarget.setTagId(C2640R.id.tag_glide);
    ...
    public /* synthetic */ void lambda$onCreate$2$MyApplication() {
        ...
        this.isExiting = true;
        ToastUtil.toast(PhoneConfig.runningActivity, getResources().getString(C2640R.string.toast_token_error));
        PhoneConfig.mainHandler.postDelayed(new Runnable() { // from class: com.ipotensic.potensicpro.-$$Lambda$MyApplication$kUUd463n_iSYETI3V16mHiU_Jqg
            @Override // java.lang.Runnable
            public final void run() {
    ...
    public /* synthetic */ void lambda$onCreate$1$MyApplication() {
        ...
        PhoneConfig.mainHandler.postDelayed(new Runnable() { // from class: com.ipotensic.potensicpro.-$$Lambda$MyApplication$veXGoPxb9qszajJ4eHjZDHB5j5Y
            @Override // java.lang.Runnable
            public final void run() {
    ...
    public /* synthetic */ void lambda$new$3$MyApplication() {
        try {
            if (PhoneConfig.isDownloadFromChinaMarket) {
                UomHandler.getInstance().init();
            }
            ...
            if (appFirstInstall) {
                if (getString(C2640R.string.is_ft).equals(BooleanUtils.TRUE)) {
                    if (!PhoneConfig.isFt) {
            ...
            DDLog.m1686e(TAG, "app启动对sp初始化失败", e);
        }
    }
        ...
        try {
            if (Objects.equals(AppUtils.getProcessName(this), AppUtils.getPackageName(this))) {
                if (PhoneConfig.curLocal == null || !PhoneConfig.curLocal.getLanguage().equals(configuration.locale.getLanguage())) {
                    DDLog.m1684e("语言已切换:" + configuration.locale.getLanguage());
                    PhoneConfig.curLocal = configuration.locale;
                    SoundPoolPlayer.getInstance().init(this);
                }
            }
        } catch (Exception e) {
            DDLog.m1684e("切换语言出错:" + e);
        }
    }

    @Override // android.app.Application
    public void onTerminate() {
        DDLog.m1684e("onTerminate");
        if (PhoneConfig.isDownloadFromChinaMarket) {
            UomHandler.getInstance().deInit();
        }
    ...
    public void onLowMemory() {
        super.onLowMemory();
        DDLog.m1691w("onLowMemory");
    }

    @Override // android.app.Application, android.content.ComponentCallbacks2
    public void onTrimMemory(int i) {
        super.onTrimMemory(i);
        DDLog.m1691w("onTrimMemory :" + i);
    }
}