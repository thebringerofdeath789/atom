...
public abstract class CallBackString<T> {
    ...
    public void onSuccess(final int i, Call call, Response response) {
        ...
        try {
            ...
            if (string == null) {
                onError(i, new Exception(resources.getString(C1819R.string.request_error)));
                return;
            }
            DDLog.m1691w("请求结果 requestcode : " + i + ", result : " + string);
            ...
            mMainHandler.post(new Runnable() { // from class: com.ipotensic.baselib.okhttp.CallBackString.2
                ...
                public void run() {
                    ...
                    if (obj == null) {
                        CallBackString.this.onFailure(i, new Exception(resources.getString(C1819R.string.request_error)));
                        return;
                    }
                    try {
                        ...
                        e.printStackTrace();
                        DDLog.m1684e("解析出错   requstCode:" + i + " , " + string + " , " + e.getMessage());
                    }
                }
            });
        } catch (Exception e) {
            onError(i, e);
            DDLog.m1684e("请求错误:" + e.getMessage());
        }
    }
}