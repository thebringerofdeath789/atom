...
public class BadgeAnimator extends ValueAnimator {
    ...
    private BitmapFragment[][] getFragments(Bitmap bitmap, PointF pointF) {
        ...
        for (int i = 0; i < bitmapFragmentArr.length; i++) {
            for (int i2 = 0; i2 < bitmapFragmentArr[i].length; i2++) {
                ...
                bitmapFragment.f2120x = f + width2;
                bitmapFragment.f2121y = f2 + height2;
    ...
    private class BitmapFragment {
        ...
        public void updata(float f, Canvas canvas) {
            ...
            this.f2120x += this.random.nextInt(this.maxSize) * 0.1f * (this.random.nextFloat() - 0.5f);
            float nextInt = this.f2121y + (this.random.nextInt(this.maxSize) * 0.1f * (this.random.nextFloat() - 0.5f));
            this.f2121y = nextInt;
            float f2 = this.f2120x;
            float f3 = this.size;
