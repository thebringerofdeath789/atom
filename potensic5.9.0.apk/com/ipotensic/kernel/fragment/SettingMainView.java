...
public final class SettingMainView extends ConstraintLayout implements RadioGroup.OnCheckedChangeListener {
    ...
    public SettingMainView(Context context, AttributeSet attributeSet, int i) {
        ...
        ViewDataBinding inflate = DataBindingUtil.inflate(LayoutInflater.from(context), C1965R.layout.view_layout_setting_main1, this, true);
    ...
    private final void switchFragment(int position) {
        ...
        fragmentManager.beginTransaction().replace(C1965R.id.fl_content, getFragmentByPosition(position)).commitAllowingStateLoss();
    }
    ...
    public void onCheckedChanged(RadioGroup group, int checkedId) {
        if (checkedId == C1965R.id.tv_security) {
            showFragment(getFragmentByPosition(0));
            return;
        }
        if (checkedId == C1965R.id.tv_calibration) {
            showFragment(getFragmentByPosition(1));
            return;
        }
        if (checkedId == C1965R.id.tv_control) {
            showFragment(getFragmentByPosition(2));
            return;
        }
        if (checkedId == C1965R.id.tv_camera) {
            showFragment(getFragmentByPosition(3));
        } else if (checkedId == C1965R.id.tv_image_trans) {
            showFragment(getFragmentByPosition(4));
        } else if (checkedId == C1965R.id.tv_about) {
    ...
    public final void showFragment(Fragment fragment) {
        ...
        if (!Intrinsics.areEqual(this.lastFragment, fragment)) {
            ...
            if (fragment.isAdded()) {
                ...
                beginTransaction.add(C1965R.id.fl_content, fragment).commitAllowingStateLoss();
            }
        }
    }
    ...
    private final void showGuideView() {
        ...
        RadioButton radioButton = this.binding.tvSecurity;
        Intrinsics.checkExpressionValueIsNotNull(radioButton, "binding.tvSecurity");
        Builder addGuidePage = with.addGuidePage(getGuidePage(radioButton, C1965R.string.guide_settings_safety_tips));
        ...
        Builder addGuidePage2 = addGuidePage.addGuidePage(getGuidePage(radioButton2, C1965R.string.guide_settings_calibration_tips));
        RadioButton radioButton3 = this.binding.tvControl;
        Intrinsics.checkExpressionValueIsNotNull(radioButton3, "binding.tvControl");
        Builder addGuidePage3 = addGuidePage2.addGuidePage(getGuidePage(radioButton3, FlightConfig.isAtomLT() ? C1965R.string.atom_lt_guide_settings_control_tips : C1965R.string.guide_settings_control_tips));
        RadioButton radioButton4 = this.binding.tvCamera;
        Intrinsics.checkExpressionValueIsNotNull(radioButton4, "binding.tvCamera");
        Builder addGuidePage4 = addGuidePage3.addGuidePage(getGuidePage(radioButton4, C1965R.string.guide_settings_camera_tips));
        RadioButton radioButton5 = this.binding.tvAbout;
        Intrinsics.checkExpressionValueIsNotNull(radioButton5, "binding.tvAbout");
        this.guideController = addGuidePage4.addGuidePage(getGuidePage(radioButton5, C1965R.string.guide_settings_about_tips)).setOnGuideChangedListener(new OnGuideChangedListener() { // from class: com.ipotensic.kernel.fragment.SettingMainView$showGuideView$1
            ...
            public void onRemoved(Controller controller, boolean isFinish, boolean isShowClick, boolean isTopMenuDismiss, boolean dismiss) {
                ...
                viewLayoutSettingMain1Binding.rgSetting.check(C1965R.id.tv_security);
            }
            ...
            public final void onPageChanged(int i) {
                ...
                if (i == 1) {
                    viewLayoutSettingMain1Binding = SettingMainView.this.binding;
                    viewLayoutSettingMain1Binding.rgSetting.check(C1965R.id.tv_calibration);
                    return;
                }
                if (i == 2) {
                    viewLayoutSettingMain1Binding2 = SettingMainView.this.binding;
                    viewLayoutSettingMain1Binding2.rgSetting.check(C1965R.id.tv_control);
                } else if (i == 3) {
                    viewLayoutSettingMain1Binding3 = SettingMainView.this.binding;
                    viewLayoutSettingMain1Binding3.rgSetting.check(C1965R.id.tv_camera);
                } else {
                    if (i != 4) {
                        return;
                    }
                    viewLayoutSettingMain1Binding4 = SettingMainView.this.binding;
                    viewLayoutSettingMain1Binding4.rgSetting.check(C1965R.id.tv_about);
                }
            }
        }).show();
    }
    ...
    private final GuidePage getGuidePage(View view, int resId) {
        GuidePage backgroundColor = GuidePage.newInstance().addHighLight(view, new SettingRelativeGuide(C1965R.layout.guide_left_new, 5, 0, 0, 0, 0, 0, Integer.valueOf(resId), 124, null)).setEnterAnimation(AnimationUtil.enterAnimation()).setExitAnimation(AnimationUtil.exitAnimation()).setEverywhereCancelable(true).setShowCanClick(false, true, false).setBackgroundColor(ContextCompat.getColor(getContext(), C1965R.color.colorFiftyPercent));
