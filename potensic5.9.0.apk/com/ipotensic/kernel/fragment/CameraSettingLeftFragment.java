...
public class CameraSettingLeftFragment extends Fragment {
    ...
    public /* synthetic */ void lambda$initListener$0$CameraSettingLeftFragment(RadioGroup radioGroup, int i) {
        if (this.mBind.getRoot().findViewById(i).isPressed()) {
            boolean z = i == C1965R.id.rb_photo_format_raw;
    ...
    public /* synthetic */ void lambda$initListener$2$CameraSettingLeftFragment(RadioGroup radioGroup, int i) {
        if (this.mBind.getRoot().findViewById(i).isPressed()) {
            this.callback.onManualSettingSetSdcardShowCapacity(i == C1965R.id.rb_sdcard_volume);
        }
    }
    ...
    public void switchCameraMode() {
        ...
        if (CameraConfig.get().isRecodeMode()) {
            ...
            this.mBind.tvPhotoFormat.setText(C1965R.string.resolution_fps);
        } else {
            this.mBind.rgPhotoFormat.setVisibility(0);
            this.resolutionAndFpsSelectView.dismiss();
            this.mBind.tvPhotoFormat.setText(C1965R.string.photo_format);
        }
    }
    ...
    public void setPhotoFormat() {
        ...
        if (CameraConfig.get().isRaw()) {
            this.mBind.rgPhotoFormat.check(C1965R.id.rb_photo_format_raw);
        } else {
            this.mBind.rgPhotoFormat.check(C1965R.id.rb_photo_format_jpg);
        }
    }
    ...
    public void updateSdCheckState() {
        ...
        if (SPHelper.getInstance().showSdCapacity()) {
            this.mBind.rgSdcardDisplayItems.check(C1965R.id.rb_sdcard_volume);
        } else {
            this.mBind.rgSdcardDisplayItems.check(C1965R.id.rb_sdcard_estimate);
        }
    }

    private String getPhotoOrVideoCapacity() {
        if (CameraConfig.get().getCaptureMode() == CaptureMode.MODE_PHOTO) {
            return !CameraConfig.get().isSdCardAvailable() ? getContext().getString(C1965R.string.n_a) : CameraConfig.get().getPhotoRemain() + " P";
        }
        return !CameraConfig.get().isSdCardAvailable() ? getContext().getString(C1965R.string.n_a) : FormatUtil.secondToHHmmss(CameraConfig.get().getRecordRemain());
    }
