                    ...
                    PhoneConfig.mainHandler.post(new Runnable() { // from class: com.ipotensic.kernel.utils.ForceUpgradeCheck.1.1
                        ...
                        public void run() {
                            ...
                            while (it.hasNext()) {
                                ...
                                DDLog.m1684e("强制升级 当前固件：" + version.getClassname());
    ...
    public void addVersion() {
        ...
        if (upgradeVersions != null) {
            ...
            while (it.hasNext()) {
                ...
                if (version != null) {
                    DDLog.m1684e("强制升级version:" + version);
        ...
        while (it2.hasNext()) {
            ...
            if (FlightConfig.PRODUCT_CLASS_FLIGHT_CTRL.equalsIgnoreCase(next.getClassname())) {
                DDLog.m1684e("强制升级 当前飞控版本：" + FlightRevData.get().getFlightRevVersionData().getFlightControlVersion());
                DDLog.m1684e("强制升级 本地飞控本号：" + next.getVersion().trim().toLowerCase());
                ...
                DDLog.m1684e("强制升级 当前相机版本：" + CameraConfig.get().getSoftVersion());
                DDLog.m1684e("强制升级 后台相机版本：" + next.getVersion().trim().toLowerCase());
                ...
                DDLog.m1684e("强制升级 当前图传版本：" + FlightRevData.get().getFlightRevFpvData().getFpvVersion());
                DDLog.m1684e("强制升级 本地图传本号：" + next.getVersion().trim().toLowerCase());
                ...
                DDLog.m1684e("强制升级 当前云台版本：" + FlightRevData.get().getFlightRevVersionData().getGimbalVersion());
                DDLog.m1684e("强制升级 本地云台本号：" + next.getVersion().trim().toLowerCase());
                ...
                DDLog.m1684e("强制升级 当前遥控器版本：" + remoterVersion);
                DDLog.m1684e("强制升级 本地遥控器本号：" + next.getVersion().trim().toLowerCase());
                ...
                DDLog.m1684e("强制升级 当前电池版本号：" + FlightRevData.get().getFlightRevVersionData().getBatteryVersion());
                DDLog.m1684e("强制升级 本地电池版本号：" + next.getVersion().trim().toLowerCase());
                ...
                if (upgradeVersions != null) {
                    ...
                    while (it2.hasNext()) {
                        ...
                        if (FlightConfig.PRODUCT_CLASS_FLIGHT_CTRL.equalsIgnoreCase(version2.getClassname())) {
                            ...
                            DDLog.m1684e("未连飞机图传单独升级fpvVersion：" + fpvVersion);
                            ...
                            DDLog.m1684e("未连飞机图传单独升级remoteCtrlVersion11：" + remoteCtrlVersion);
                            if (remoteCtrlVersion == null) {
                                remoteCtrlVersion = SPHelper.getInstance().getRemoterCurVersion();
                                DDLog.m1684e("未连飞机图传单独升级remoteCtrlVersion22：" + remoteCtrlVersion);
                            }
                ...
                if (upgradeVersions != null) {
                    ...
                    while (it2.hasNext()) {
                        ...
                        if (FlightConfig.PRODUCT_CLASS_FLIGHT_CTRL.equalsIgnoreCase(version2.getClassname())) {
                            ...
                            String fpvVersion = FlightRevData.get().getFlightRevFpvData().getFpvVersion();
                            DDLog.m1684e("未连飞机遥控器单独升级fpvVersion11：" + fpvVersion);
                            if (fpvVersion == null) {
                                fpvVersion = SPHelper.getInstance().getFpvCurVersion();
                                DDLog.m1684e("未连飞机遥控器单独升级fpvVersion22：" + fpvVersion);
                            }
                            ...
                            String remoteCtrlVersion = FlightRevData.get().getFlightRevRemoteCtrlInfoData().getRemoteCtrlVersion();
                            DDLog.m1684e("未连飞机遥控器单独升级remoteCtrlVersion：" + remoteCtrlVersion);
                            if (!CommonUtil.hasNewVersion(remoteCtrlVersion, version2.getVersion().trim().toLowerCase())) {
                                continue;
        ...
        PhoneConfig.threadPool.execute(new Runnable() { // from class: com.ipotensic.kernel.utils.ForceUpgradeCheck.5
            ...
            public void run() {
                ...
                if (version != null && version.getIsforce() == 0 && CommonUtil.hasNewVersion(SPHelper.getInstance().getBigPackageVersion(), version.getVersion().trim().toLowerCase())) {
                    ...
                    DDLog.m1684e("检测是否需要大包强制升级:" + ForceUpgradeCheck.this.needForceUpgradeTips);
                }
            }
        });
    }
            ...
            PhoneConfig.threadPool.execute(new Runnable() { // from class: com.ipotensic.kernel.utils.ForceUpgradeCheck.6
                ...
                public void run() {
                    ...
                    if (version != null) {
                        ...
                        DDLog.m1684e("大包当前版本:" + bigPackageVersion + ", 后台版本：" + version.getVersion().trim());
                        if (version.getProductclass() != null) {
                            if (version.getProductclass().equalsIgnoreCase(!TextUtils.isEmpty(FlightConfig.getBigPackageLastProductClass()) ? FlightConfig.getBigPackageLastProductClass() : FlightConfig.getLastProductClass()) && CommonUtil.hasNewVersion(bigPackageVersion, version.getVersion().trim().toLowerCase())) {
                                if (ForceUpgradeCheck.this.isFileDownloaded(version)) {
                                    DDLog.m1684e("大包本地已有固件：" + bigPackageVersion + ", 本地version: " + version.getVersion());
