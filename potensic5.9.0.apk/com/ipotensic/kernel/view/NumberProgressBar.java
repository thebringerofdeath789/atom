...
public class NumberProgressBar extends View {
    ...
    public NumberProgressBar(Context context, AttributeSet attributeSet, int i) {
        ...
        this.mSuffix = "%";
        this.mPrefix = "";
        int rgb = Color.rgb(66, 145, TelnetCommand.NOP);
        this.default_text_color = rgb;
        int rgb2 = Color.rgb(66, 145, TelnetCommand.NOP);
        ...
        this.default_round_radius = dp2px(4.0f);
        TypedArray obtainStyledAttributes = context.getTheme().obtainStyledAttributes(attributeSet, C1965R.styleable.NumberProgressBar, i, 0);
        this.mReachedBarColor = obtainStyledAttributes.getColor(C1965R.styleable.NumberProgressBar_progress_reached_color, rgb2);
        this.mUnreachedBarColor = obtainStyledAttributes.getColor(C1965R.styleable.NumberProgressBar_progress_unreached_color, rgb3);
        this.mTextColor = obtainStyledAttributes.getColor(C1965R.styleable.NumberProgressBar_progress_text_color, rgb);
        this.mTextSize = obtainStyledAttributes.getDimension(C1965R.styleable.NumberProgressBar_progress_text_size, sp2px);
        this.mReachedBarHeight = obtainStyledAttributes.getDimension(C1965R.styleable.NumberProgressBar_progress_reached_bar_height, dp2px);
        this.mUnreachedBarHeight = obtainStyledAttributes.getDimension(C1965R.styleable.NumberProgressBar_progress_unreached_bar_height, dp2px2);
        this.mOffset = obtainStyledAttributes.getDimension(C1965R.styleable.NumberProgressBar_progress_text_offset, dp2px3);
        if (obtainStyledAttributes.getInt(C1965R.styleable.NumberProgressBar_progress_text_visibility, 0) != 0) {
            this.mIfDrawText = false;
        }
        setProgress(obtainStyledAttributes.getInt(C1965R.styleable.NumberProgressBar_progress_current, 0));
        setMax(obtainStyledAttributes.getInt(C1965R.styleable.NumberProgressBar_progress_max, 100));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.mDrawReachedBar) {
            ...
            float f = this.default_round_radius;
            canvas.drawRoundRect(rectF, f, f, this.mReachedBarPaint);
        }
        if (this.mDrawUnreachedBar) {
            RectF rectF2 = this.mUnreachedRectF;
            float f2 = this.default_round_radius;
            canvas.drawRoundRect(rectF2, f2, f2, this.mUnreachedBarPaint);
        }
        if (this.mIfDrawText) {
