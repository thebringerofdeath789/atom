...
public class MiniLandSlideUnlockDialog extends BaseSyncDialog implements View.OnClickListener {
    ...
    public MiniLandSlideUnlockDialog(Context context, SlideUnlockListener slideUnlockListener, boolean z) {
        super(context, C1965R.layout.view_dialog_mini_slide_land);
        this.isCountdown = false;
        this.listener = slideUnlockListener;
        setCanceledOnTouchOutside(false);
        getWindow().setDimAmount(0.5f);
        this.isCountdown = z;
        TextView textView = (TextView) findViewById(C1965R.id.tv_count_down);
        this.textCountDown = textView;
        this.landSlideUnlockView.switchCenterCircleBpToNoContent(z);
        String string = getContext().getString(C1965R.string.dialog_mini_land_content);
        if (z) {
            if (PhoneConfig.isFt) {
                string = String.format(getContext().getString(C1965R.string.short_distance_land_notice), "66ft", "16ft");
            } else {
                string = String.format(getContext().getString(C1965R.string.short_distance_land_notice), "20m", "5m");
            }
        }
        ((TextView) findViewById(C1965R.id.tv_mini_takeoff_content)).setText(string);
        textView.setVisibility(z ? 0 : 8);
    }

    @Override // com.ipotensic.baselib.base.BaseDialog
    protected void initView(Context context) {
        ImageButton imageButton = (ImageButton) findViewById(C1965R.id.btn_close_land);
        this.ibClose = imageButton;
        imageButton.setOnClickListener(this);
        MiniLandSlideUnlockView miniLandSlideUnlockView = (MiniLandSlideUnlockView) findViewById(C1965R.id.btn_mini_slide_unlock_land);
