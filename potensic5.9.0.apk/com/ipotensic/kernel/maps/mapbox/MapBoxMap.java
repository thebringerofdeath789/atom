...
public class MapBoxMap extends BaseMap<LatLng> implements OnMapReadyCallback, MapboxMap.OnMapClickListener, MapboxMap.OnScaleListener, MapView.OnCameraDidChangeListener, EventDispatcher.OnEventListener, MapboxMap.OnMoveListener {
    ...
    public void initMapStyle(Style style) {
        ...
        this.style.addImage("marker-image-device", BitmapUtils.getDrawableFromRes(this.context, C1965R.mipmap.icon_remoter_point_normal));
        this.style.addImage("marker-image-system", BitmapUtils.getDrawableFromRes(this.context, C1965R.mipmap.icon_fb_badges));
        this.style.addImage("marker-image-device-pair", BitmapUtils.getDrawableFromRes(this.context, C1965R.mipmap.icon_remoter_point_pair));
        this.style.addImage("marker-image-plane", BitmapUtils.getDrawableFromRes(this.context, C1965R.mipmap.icon_drone_enable_normal));
        this.style.addImage("marker-image-plane-pair", BitmapUtils.getDrawableFromRes(this.context, C1965R.mipmap.icon_drone_enable_pair));
        this.style.addImage("marker-image-home", BitmapUtils.getDrawableFromRes(this.context, C1965R.mipmap.mapbox_marker_icon_home_position));
    ...
    private void checkPointFlyLine(double d, double d2) {
        ...
        for (int i = 0; i < this.lines.size(); i++) {
            DDLog.m1684e("pointFly - > " + this.lines.get(i).getLatLngs().contains(new LatLng(d, d2)) + ", lat -> " + d + ", lng -> " + d2);
        }
    }
    ...
    public void drawPoint() {
        if (this.isStyleLoaded) {
            ...
            if (this.pointList.size() >= i) {
                ToastUtil.showImageTop(this.context, String.format(this.context.getString(C1965R.string.toast_map_max_num), Integer.valueOf(i)), C1965R.mipmap.icon_toast_prompt);
                return;
            }
            if (FlightRevData.get().getFlightRevStateData().getMode() != 0 && FlightRevData.get().getFlightRevStateData().isReceiveGps()) {
                String format = String.format(this.context.getString(C1965R.string.toast_map_point_tips), Integer.valueOf(FlightConfig.getMaxPointDistance()));
                ...
                if (latLng == null) {
                    if (TurfMeasurement.distance(Point.fromLngLat(this.curLng, this.curLat), Point.fromLngLat(this.point.getLongitude(), this.point.getLatitude()), TurfConstants.UNIT_METERS) > FlightConfig.getMaxPointDistance()) {
                        ToastUtil.showImageTop(this.context, format, C1965R.mipmap.icon_toast_prompt);
                        return;
                    }
    ...
    private void setDrawPoint(LatLng latLng, int i) {
        ...
        View inflate = LayoutInflater.from(this.context).inflate(C1965R.layout.view_map_mark, (ViewGroup) null);
        ((TextView) inflate.findViewById(C1965R.id.tv_num)).setText(String.valueOf(i));
    ...
    public void drawLine(ArrayList<LatLng> arrayList, boolean z) {
        ...
        this.lines.add(this.lineManager.create((LineManager) new LineOptions().withLineJoin(Property.LINE_JOIN_BEVEL).withLatLngs(arrayList).withLineColor(ColorUtils.colorToRgbaString(this.context.getResources().getColor(C1965R.color.map_fold_line))).withLineWidth(Float.valueOf(2.5f))));
        arrayList.remove(0);
    }
    ...
    public void drawCircle(float f) {
        ...
        Circle create = this.circleManager.create((CircleManager) new CircleOptions().withLatLng(new LatLng(this.flyLat, this.flyLng)).withCircleColor(ColorUtils.colorToRgbaString(this.context.getResources().getColor(C1965R.color.color_map_circle))));
    ...
    public void fly() {
        ...
        if (FlightRevData.get().getFlightRevStateData().isReturning()) {
            ...
            ToastUtil.toast(appCompatActivity, appCompatActivity.getString(C1965R.string.returning_switch_mode_unavailable));
        ...
        if (flightRevFlightInfoData.getVerticalDistance() < 5.0d) {
            ...
            ToastUtil.toast(appCompatActivity2, appCompatActivity2.getString(C1965R.string.setting_height_low_follow_fail));
        } else if (FlightConfig.isOldProduct()) {
            AppCompatActivity appCompatActivity3 = this.context;
            new SlideUnlockDialog(appCompatActivity3, appCompatActivity3.getResources().getString(C1965R.string.dialog_go), this.context.getResources().getString(C1965R.string.dialog_go_describe), new SlideUnlockDialog.SlideResultListener() { // from class: com.ipotensic.kernel.maps.mapbox.MapBoxMap.3
                @Override // com.ipotensic.kernel.view.dialog.SlideUnlockDialog.SlideResultListener
                public void dealEvent() {
                    if (flightRevFlightInfoData.getVerticalDistance() < 5.0d) {
                        ToastUtil.toast(MapBoxMap.this.context, MapBoxMap.this.context.getString(C1965R.string.setting_height_low_follow_fail));
                    } else {
            ...
            new MiniTakeoffSlideUnlockDialog(appCompatActivity4, appCompatActivity4.getResources().getString(C1965R.string.dialog_go), this.context.getResources().getString(C1965R.string.dialog_go_describe), true, false, new MiniTakeoffSlideUnlockDialog.SlideUnlockListener() { // from class: com.ipotensic.kernel.maps.mapbox.MapBoxMap.4
    ...
    public void saveMarkFlight() {
        ...
        flightRecordBean.saveAsync().listen(new SaveCallback() { // from class: com.ipotensic.kernel.maps.mapbox.MapBoxMap.6
            ...
            public void onFinish(boolean z) {
                if (z) {
                    FlightConfig.isPointFlyFinished = false;
                    ToastUtil.showImageTop(MapBoxMap.this.context, MapBoxMap.this.context.getResources().getString(C1965R.string.toast_map_save), C1965R.mipmap.icon_toast_successful);
                }
            }
        });
    ...
    public void setZoomScale(float f) {
        ...
        if (mapboxMap != null) {
            try {
                ...
                DDLog.m1684e("地图scale出错 ：" + e.getMessage());
            }
        }
    }
