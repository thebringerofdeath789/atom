...
public class BDLocationService implements ILocationService, OnNetworkChangeListener {
    ...
    public void init() {
        try {
            ...
            DDLog.m1685e(this.TAG, "开启定位出错:" + e.getMessage());
        }
    }

    @Override // com.ipotensic.kernel.services.ILocationService
    public void start() {
        DDLog.m1685e(this.TAG, "开始定位...");
        if (this.client == null) {
            init();
        }
    ...
    public void testReGeo(double d, double d2) {
        if (NetworkUtils.isNetConnected(PhoneConfig.applicationContext)) {
            MapboxGeocoding.builder().accessToken(PhoneConfig.applicationContext.getString(C1965R.string.mapbox_access_token)).query(Point.fromLngLat(d, d2)).geocodingTypes(GeocodingCriteria.TYPE_COUNTRY).build().enqueueCall(new Callback<GeocodingResponse>() { // from class: com.ipotensic.kernel.services.BDLocationService.2
                ...
                public void onResponse(Call<GeocodingResponse> call, Response<GeocodingResponse> response) {
                    ...
                    DDLog.m1684e("国家简写：" + removeSpecialCharacters.toUpperCase());
                }

                @Override // retrofit2.Callback
                public void onFailure(Call<GeocodingResponse> call, Throwable th) {
                    DDLog.m1684e("国家 请求失败：" + th);
                }
            });
        }
    }
    ...
    public void refresh(final BDLocation bDLocation) {
        ...
        DDLog.m1685e(this.TAG, "当前位置:" + bDLocation.getLatitude() + "," + bDLocation.getLongitude() + "," + bDLocation.getRadius());
