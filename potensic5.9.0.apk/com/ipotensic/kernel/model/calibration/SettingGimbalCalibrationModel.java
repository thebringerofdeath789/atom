...
public class SettingGimbalCalibrationModel {
    ...
    public ObservableInt calibrationResultResId = new ObservableInt(C1965R.mipmap.img_gimbal_calibration_success);
    ...
    public void startCalibration(View view) {
        DDLog.m1684e("发送云台校准指令0");
        if (checkErrorStatus()) {
            return;
        }
        this.isStopRevData = false;
        DDLog.m1684e("发送云台校准指令1");
        sendSetting();
    }
    ...
    private boolean checkErrorStatus() {
        ...
        if (error_status == 4) {
            ...
            ToastUtil.toast(fragmentActivity, fragmentActivity.getString(C1965R.string.warning_the_gimbal_is_stuck_briefly_tips));
            return true;
        }
        if (error_status == 5) {
            FragmentActivity fragmentActivity2 = this.activity;
            ToastUtil.toast(fragmentActivity2, fragmentActivity2.getString(C1965R.string.warning_flight_interface_the_gimbal_motor_is_overloaded_tips));
            return true;
        }
        if (error_status == 1 || error_status == 2) {
            FragmentActivity fragmentActivity3 = this.activity;
            ToastUtil.toast(fragmentActivity3, fragmentActivity3.getString(C1965R.string.txt_error_gimbal_error));
            return true;
        }
        if (error_status == 3 && FlightRevData.get().getFlightRevStateData().isUnLock()) {
            FragmentActivity fragmentActivity4 = this.activity;
            ToastUtil.toast(fragmentActivity4, fragmentActivity4.getString(C1965R.string.txt_error_flighting_unavailable));
            return true;
        }
        ...
        ToastUtil.toast(fragmentActivity5, fragmentActivity5.getString(C1965R.string.txt_error_flighting_unavailable));
        return true;
    }
    ...
    public void update(FlightRevGimbalSettingData flightRevGimbalSettingData) {
        ...
        if (flightRevGimbalSettingData.getGimbalCalibration() == 3) {
            ...
            this.resultStr.set(this.activity.getString(C1965R.string.calibration_successful_wait_reboot));
            setCalibrationResultResourceId(C1965R.mipmap.img_gimbal_calibration_success);
            this.calibrateResult.setValue(1);
            return;
        }
        if (flightRevGimbalSettingData.getGimbalCalibration() == 4) {
            setViewVisible(false, true, false, true);
            this.resultStr.set(this.activity.getString(C1965R.string.calibration_failed));
            setCalibrationResultResourceId(C1965R.mipmap.img_gimbal_calibration_fail);
            this.startBtnStr.set(this.activity.getString(C1965R.string.gimbral_calibration_retry));
    ...
    public void resetUI(boolean z) {
        ...
        this.titleStr.set(this.activity.getString(C1965R.string.setting_calibration_gimbal));
        this.resultStr.set(this.activity.getString(C1965R.string.calibration_successful_wait_reboot));
        this.startBtnStr.set(this.activity.getString(C1965R.string.compass_start_calibration));
    }
}