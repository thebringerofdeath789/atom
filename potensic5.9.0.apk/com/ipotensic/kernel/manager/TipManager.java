...
public class TipManager implements EventDispatcher.OnEventListener {
    ...
    public TipManager(AppCompatActivity appCompatActivity, LinearLayout linearLayout) {
        ...
        handlerThread.start();
        this.mHandler = new HandlerC23711(handlerThread.getLooper(), appCompatActivity);
    }

    /* renamed from: com.ipotensic.kernel.manager.TipManager$1 */
    class HandlerC23711 extends Handler {
        final /* synthetic */ AppCompatActivity val$activity;

        /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
        HandlerC23711(Looper looper, AppCompatActivity appCompatActivity) {
            super(looper);
        ...
        public void handleMessage(Message message) {
            ...
            if (remove.getStrId() != -1 && !TipManager.this.errorTipsManager.isContain(remove.getStrId())) {
                TipManager.this.errorTipsManager.enqueue(remove.getStrId(), remove.getText());
            }
                ...
                if (!ActivityHelper.getInstance().isAppInForeground()) {
                    ...
                    SoundMediaPlayer.get().play(this.val$activity, remove.getRawId(), new MediaPlayer.OnCompletionListener() { // from class: com.ipotensic.kernel.manager.-$$Lambda$TipManager$1$GRyg-CpVZ4Wf247KjXhqDbdcSnw
                        ...
                        public final void onCompletion(MediaPlayer mediaPlayer) {
                            TipManager.HandlerC23711.this.lambda$handleMessage$0$TipManager$1(mediaPlayer);
                        }
                    });
                }
            }
    ...
    public void addOnlyOnce(SoundTip soundTip) {
        DDLog.m1685e(this.TAG, "音频 = " + soundTip.getRawId() + ", 提示 = " + soundTip.getStrId() + ", size = " + this.tipsQueue.size());
    ...
    public void onEvent(EventID eventID, Event event) {
        switch (C23722.$SwitchMap$com$ipotensic$baselib$dispatcher$EventID[eventID.ordinal()]) {
            case 1:
                addOnlyOnce(new SoundTip(C1965R.raw.audio_attitude_mode, -1));
                break;
            case 2:
            case 3:
                stopAndRemove(new SoundTip(C1965R.raw.audio_sd_pullout, -1));
                addOnlyOnce(new SoundTip(C1965R.raw.audio_sd_insertion, -1));
                break;
            case 4:
            case 5:
                stopAndRemove(new SoundTip(C1965R.raw.audio_sd_insertion, -1));
                addOnlyOnce(new SoundTip(C1965R.raw.audio_sd_pullout, -1));
                break;
            case 6:
                addOnlyOnce(new SoundTip(C1965R.raw.audio_sd_full, -1));
                break;
            case 7:
            case 8:
                addOnlyOnce(new SoundTip(C1965R.raw.audio_sd_unknown_type, -1));
                break;
            case 9:
                DDLog.m1684e("语音提示未检测到SD卡");
                addOnlyOnce(new SoundTip(C1965R.raw.audio_no_sd_card, -1));
                break;
            case 10:
                int i = event.arg1;
                int i2 = event.arg2;
                if (i == 1) {
                    if (!this.isCameraTooHighTemp) {
                        ...
                        DDLog.m1684e("相机二级高温");
                        addOnlyOnce(new SoundTip(C1965R.raw.audio_camera_is_overheated, -1, true));
                        break;
                    }
                } else if (i == 2 && !this.isCameraStopWork) {
                    this.isCameraStopWork = true;
                    DDLog.m1684e("相机三级高温");
                    addOnlyOnce(new SoundTip(C1965R.raw.audio_camera_temperature_has_exceeded_the_limit, -1, true));
                    break;
                }
                break;
            case 11:
                addOnlyOnce(new SoundTip(C1965R.raw.audio_returning_not_available, -1, false));
                break;
            case 12:
                if (!FlightConfig.isOldProduct()) {
                    DDLog.m1684e("返航点已刷新 语音提示");
                    addOnlyOnce(new SoundTip(C1965R.raw.return_point_has_been_refreshed, -1, false));
                    break;
                }
                break;
            case 13:
                ...
                if (flightRevStateData.isFlight() && flightRevStateData.isReturning() && !flightRevStateData.isReturnCountdown() && !flightRevStateData.isBatteryHighPress() && !flightRevStateData.isIs1BatteryDamaged() && !flightRevStateData.isBatteryJump() && this.data.isReturning() != flightRevStateData.isReturning()) {
                    addOnlyOnce(new SoundTip(C1965R.raw.audio_drone_is_returning, -1, true));
                } else {
                    ...
                    if (flightRevStateData2 != null && flightRevStateData2.isReturning() != flightRevStateData.isReturning() && !flightRevStateData.isReturnCountdown()) {
                        stopAndRemove(new SoundTip(C1965R.raw.audio_drone_is_returning, -1, true));
                    }
                }
                if (flightRevStateData.isFlight() && flightRevStateData.isReturnCountdown() && flightRevStateData.isReturnCountdown() != this.data.isReturnCountdown()) {
                    addOnlyOnce(new SoundTip(C1965R.raw.audio_low_power_return_soon, -1, true));
                } else if (this.data.isReturnCountdown() != flightRevStateData.isReturnCountdown()) {
                    stopAndRemove(new SoundTip(C1965R.raw.audio_low_power_return_soon, -1, true));
                }
                ...
                if (flightRevStateData.isNewBattery()) {
                    if (flightRevStateData.isIs1BatteryDamaged() && this.data.isIs1BatteryDamaged() != flightRevStateData.isIs1BatteryDamaged()) {
                        stopAndRemove(flightRevStateData.isFlight() ? new SoundTip(-1, C1965R.string.error_battery_no_takeoff_fault) : new SoundTip(C1965R.raw.audio_battery_pressure_too_large, C1965R.string.error_battery_flight_fault, true));
                        addOnlyOnce(flightRevStateData.isFlight() ? new SoundTip(C1965R.raw.audio_battery_pressure_too_large, C1965R.string.error_battery_flight_fault, true) : new SoundTip(-1, C1965R.string.error_battery_no_takeoff_fault));
                    } else if (this.data.isIs1BatteryDamaged() != flightRevStateData.isIs1BatteryDamaged()) {
                        stopAndRemove(new SoundTip(C1965R.raw.audio_battery_pressure_too_large, C1965R.string.error_battery_flight_fault, true));
                        stopAndRemove(new SoundTip(-1, C1965R.string.error_battery_no_takeoff_fault));
                    }
                    if (flightRevStateData.isFlight() && flightRevStateData.isIs2BatteryDamaged() && flightRevStateData.isIs2BatteryDamaged() != this.data.isIs2BatteryDamaged()) {
                        addOnlyOnce(new SoundTip(C1965R.raw.audio_battery_abnormal_emergency_landing, -1));
                    } else if (this.data.isIs2BatteryDamaged() != flightRevStateData.isIs2BatteryDamaged()) {
                        stopAndRemove(new SoundTip(C1965R.raw.audio_battery_abnormal_emergency_landing, -1));
                    }
                    if (flightRevStateData.isFlight() && flightRevStateData.isBatteryJump() && flightRevStateData.isBatteryJump() != this.data.isBatteryJump()) {
                        addOnlyOnce(new SoundTip(C1965R.raw.audio_battery_jumps_abnormally_trigger_return_home, -1, true));
                    } else if (this.data.isBatteryJump() != flightRevStateData.isBatteryJump()) {
                        stopAndRemove(new SoundTip(C1965R.raw.audio_battery_jumps_abnormally_trigger_return_home, -1, true));
                    }
                ...
                if (flightRevStateData.isGeoCalibrationFailureFlag() && flightRevStateData.isGeoCalibrationFailureFlag() != this.data.isGeoCalibrationFailureFlag()) {
                    addOnlyOnce(new SoundTip(-1, C1965R.string.toast_geo_calibration_fail));
                }
                if (flightRevStateData.isTofInvalid() && flightRevStateData.isTofInvalid() != this.data.isTofInvalid()) {
                    addOnlyOnce(new SoundTip(-1, C1965R.string.tof_invalid));
                }
                try {
                    this.data = flightRevStateData.m2625clone();
                    break;
            ...
            case 14:
                ...
                DDLog.m1685e(this.TAG, "connect state:" + booleanValue);
                if (booleanValue) {
                    if (this.data == null) {
            ...
            case 18:
                ...
                break;
            case 19:
                addOnlyOnce(new SoundTip(C1965R.raw.audio_start_horizontal_calibration, -1, false));
                break;
            case 20:
                addOnlyOnce(new SoundTip(C1965R.raw.audio_start_vertical_calibration, -1, false));
                break;
            case 21:
                addOnlyOnce(new SoundTip(C1965R.raw.audio_calibration_success, -1, false));
                break;
            case 22:
                addOnlyOnce(new SoundTip(C1965R.raw.audio_calibration_fail, -1, false));
                break;
        }
    ...
    private void setState(State state) {
        DDLog.m1685e(this.TAG, "连接状态state—-》 now " + this.state);
        if (state != this.state) {
            DDLog.m1685e(this.TAG, "连接状态state—-》 before " + this.state);
            this.state = state;
            DDLog.m1685e(this.TAG, "连接状态state—-》 after " + this.state);
            switch (C23722.$SwitchMap$com$ipotensic$baselib$enums$State[this.state.ordinal()]) {
                case 1:
                    addOnlyOnce(new SoundTip(C1965R.raw.audio_please_connect_the_drone, -1, true));
                    break;
                case 2:
                case 3:
                case 4:
                    stopAndRemove(new SoundTip(C1965R.raw.audio_please_connect_the_drone, -1, true));
                    addOnlyOnce(new SoundTip(C1965R.raw.audio_remote_control_not_connected, -1, true));
                    break;
                case 5:
                case 6:
                    stopAndRemove(new SoundTip(C1965R.raw.audio_remote_control_not_connected, -1, true));
                    addOnlyOnce(new SoundTip(C1965R.raw.audio_remote_control_connected, -1, true));
                    break;
                case 7:
                case 8:
                    stopAndRemove(new SoundTip(C1965R.raw.audio_remote_control_not_connected, -1, true));
                    addOnlyOnce(new SoundTip(C1965R.raw.audio_plane_connecting, -1, true));
                    break;
            }
        }
    }

    /* renamed from: com.ipotensic.kernel.manager.TipManager$2 */
    static /* synthetic */ class C23722 {
