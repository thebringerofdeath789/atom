...
public class DecoderThread {
    ...
    private final Handler.Callback callback = new Handler.Callback() { // from class: com.journeyapps.barcodescanner.DecoderThread.1
        ...
        public boolean handleMessage(Message message) {
            if (message.what == C1783R.id.zxing_decode) {
                DecoderThread.this.decode((SourceData) message.obj);
                return true;
            }
            if (message.what != C1783R.id.zxing_preview_failed) {
                return true;
            }
    ...
    private final PreviewCallback previewCallback = new PreviewCallback() { // from class: com.journeyapps.barcodescanner.DecoderThread.2
        ...
        public void onPreview(SourceData sourceData) {
            synchronized (DecoderThread.this.LOCK) {
                if (DecoderThread.this.running) {
                    DecoderThread.this.handler.obtainMessage(C1783R.id.zxing_decode, sourceData).sendToTarget();
                }
            }
        }
        ...
        public void onPreviewError(Exception exc) {
            synchronized (DecoderThread.this.LOCK) {
                if (DecoderThread.this.running) {
                    DecoderThread.this.handler.obtainMessage(C1783R.id.zxing_preview_failed).sendToTarget();
                }
            }
        }
    };
    ...
    public void decode(SourceData sourceData) {
        ...
        if (decode != null) {
            ...
            if (this.resultHandler != null) {
                Message obtain = Message.obtain(this.resultHandler, C1783R.id.zxing_decode_succeeded, new BarcodeResult(decode, sourceData));
            ...
            if (handler != null) {
                Message.obtain(handler, C1783R.id.zxing_decode_failed).sendToTarget();
            }
        }
        if (this.resultHandler != null) {
            Message.obtain(this.resultHandler, C1783R.id.zxing_possible_result_points, BarcodeResult.transformResultPoints(this.decoder.getPossibleResultPoints(), sourceData)).sendToTarget();
        }
        requestNextPreview();
    }
}