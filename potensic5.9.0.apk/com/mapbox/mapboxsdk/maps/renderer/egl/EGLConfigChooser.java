...
public class EGLConfigChooser implements GLSurfaceView.EGLConfigChooser {
    ...
    public EGLConfig chooseConfig(EGL10 egl10, EGLDisplay eGLDisplay) {
        ...
        if (numberOfConfigurations[0] < 1) {
            Logger.m1756e(TAG, "eglChooseConfig() returned no configs.");
        }
        ...
        if (chooseBestMatchConfig == null) {
            Logger.m1756e(TAG, "No config chosen");
        }
    ...
    private int[] getNumberOfConfigurations(EGL10 egl10, EGLDisplay eGLDisplay, int[] iArr) {
        ...
        if (!egl10.eglChooseConfig(eGLDisplay, iArr, null, 0, iArr2)) {
            Logger.m1756e(TAG, String.format(MapboxConstants.MAPBOX_LOCALE, "eglChooseConfig(NULL) returned error %d", Integer.valueOf(egl10.eglGetError())));
        }
        return iArr2;
    }
    ...
    private EGLConfig[] getPossibleConfigurations(EGL10 egl10, EGLDisplay eGLDisplay, int[] iArr, int[] iArr2) {
        ...
        if (!egl10.eglChooseConfig(eGLDisplay, iArr, eGLConfigArr, iArr2[0], iArr2)) {
            Logger.m1756e(TAG, String.format(MapboxConstants.MAPBOX_LOCALE, "eglChooseConfig() returned error %d", Integer.valueOf(egl10.eglGetError())));
        }
        return eGLConfigArr;
    }
    ...
    private EGLConfig chooseBestMatchConfig(EGL10 egl10, EGLDisplay eGLDisplay, EGLConfig[] eGLConfigArr) {
        ...
        if (arrayList.size() == 0) {
            Logger.m1756e(TAG, "No matching configurations after filtering");
            return null;
        }
        C1Config c1Config = (C1Config) arrayList.get(0);
        if (c1Config.isCaveat) {
            Logger.m1762w(TAG, "Chosen config has a caveat.");
        }
        if (c1Config.isNotConformant) {
            Logger.m1762w(TAG, "Chosen config is not conformant.");
        }
    ...
    private int getConfigAttr(EGL10 egl10, EGLDisplay eGLDisplay, EGLConfig eGLConfig, int i) {
        ...
        if (!egl10.eglGetConfigAttrib(eGLDisplay, eGLConfig, i, iArr)) {
            Logger.m1756e(TAG, String.format(MapboxConstants.MAPBOX_LOCALE, "eglGetConfigAttrib(%d) returned error %d", Integer.valueOf(i), Integer.valueOf(egl10.eglGetError())));
        }
        return iArr[0];
    }
    ...
    private int[] getConfigAttributes() {
        ...
        Logger.m1758i(TAG, String.format("In emulator: %s", Boolean.valueOf(z)));
        int[] iArr = new int[25];
        iArr[0] = 12327;
        iArr[1] = 12344;
        iArr[2] = 12339;
