...
class TextureViewRenderThread extends Thread implements TextureView.SurfaceTextureListener {
                ...
                if (remove != null) {
                    ...
                    if (z) {
                        ...
                        if (swap == 12288) {
                            ...
                            Logger.m1762w(TAG, String.format("eglSwapBuffer error: %s. Waiting or new surface", Integer.valueOf(swap)));
                            synchronized (this.lock) {
                                this.surface = null;
                                this.destroySurface = true;
                            }
                        } else {
                            Logger.m1762w(TAG, "Context lost. Waiting for re-aquire");
                            synchronized (this.lock) {
                                this.surface = null;
                                this.destroySurface = true;
    ...
    private static class EGLHolder {
        ...
        boolean createSurface() {
            ...
            if (eGLSurface == null || eGLSurface == EGL10.EGL_NO_SURFACE) {
                if (this.egl.eglGetError() == 12299) {
                    Logger.m1756e(TextureViewRenderThread.TAG, "createWindowSurface returned EGL_BAD_NATIVE_WINDOW.");
                }
                return false;
            }
            return makeCurrent();
        ...
        boolean makeCurrent() {
            ...
            Logger.m1762w(TextureViewRenderThread.TAG, String.format("eglMakeCurrent: %s", Integer.valueOf(this.egl.eglGetError())));
            return false;
        }
        ...
        public void destroySurface() {
            ...
            if (!this.egl.eglDestroySurface(this.eglDisplay, this.eglSurface)) {
                Logger.m1762w(TextureViewRenderThread.TAG, String.format("Could not destroy egl surface. Display %s, Surface %s", this.eglDisplay, this.eglSurface));
            }
            this.eglSurface = EGL10.EGL_NO_SURFACE;
        }
        ...
        public void destroyContext() {
            ...
            if (!this.egl.eglDestroyContext(this.eglDisplay, this.eglContext)) {
                Logger.m1762w(TextureViewRenderThread.TAG, String.format("Could not destroy egl context. Display %s, Context %s", this.eglDisplay, this.eglContext));
            }
            this.eglContext = EGL10.EGL_NO_CONTEXT;
        }
        ...
        private void terminate() {
            ...
            if (!this.egl.eglTerminate(this.eglDisplay)) {
                Logger.m1762w(TextureViewRenderThread.TAG, String.format("Could not terminate egl. Display %s", this.eglDisplay));
            }
