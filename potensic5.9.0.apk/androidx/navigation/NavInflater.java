...
public final class NavInflater {
        ...
        while (true) {
            ...
            if (next == 2 && depth <= depth2) {
                ...
                if (TAG_ARGUMENT.equals(name)) {
                    ...
                    TypedArray obtainAttributes = resources.obtainAttributes(attributeSet, C0428R.styleable.NavInclude);
                    ((NavGraph) createDestination).addDestination(inflate(obtainAttributes.getResourceId(C0428R.styleable.NavInclude_graph, 0)));
    ...
    private void inflateArgumentForDestination(Resources resources, NavDestination navDestination, AttributeSet attributeSet, int i) throws XmlPullParserException {
        TypedArray obtainAttributes = resources.obtainAttributes(attributeSet, C0428R.styleable.NavArgument);
        String string = obtainAttributes.getString(C0428R.styleable.NavArgument_android_name);
        if (string == null) {
    ...
    private NavArgument inflateArgument(TypedArray typedArray, Resources resources, int i) throws XmlPullParserException {
        ...
        builder.setIsNullable(typedArray.getBoolean(C0428R.styleable.NavArgument_nullable, false));
        ...
        String string = typedArray.getString(C0428R.styleable.NavArgument_argType);
        Object obj = null;
        NavType<?> fromArgType = string != null ? NavType.fromArgType(string, resources.getResourcePackageName(i)) : null;
        if (typedArray.getValue(C0428R.styleable.NavArgument_android_defaultValue, typedValue)) {
            if (fromArgType == NavType.ReferenceType) {
                ...
                obj = typedArray.getString(C0428R.styleable.NavArgument_android_defaultValue);
            } else {
    ...
    private void inflateDeepLink(Resources resources, NavDestination navDestination, AttributeSet attributeSet) {
        TypedArray obtainAttributes = resources.obtainAttributes(attributeSet, C0428R.styleable.NavDeepLink);
        String string = obtainAttributes.getString(C0428R.styleable.NavDeepLink_uri);
    ...
    private void inflateAction(Resources resources, NavDestination navDestination, AttributeSet attributeSet, XmlResourceParser xmlResourceParser, int i) throws IOException, XmlPullParserException {
        int depth;
        TypedArray obtainAttributes = resources.obtainAttributes(attributeSet, C0428R.styleable.NavAction);
        int resourceId = obtainAttributes.getResourceId(C0428R.styleable.NavAction_android_id, 0);
        NavAction navAction = new NavAction(obtainAttributes.getResourceId(C0428R.styleable.NavAction_destination, 0));
        NavOptions.Builder builder = new NavOptions.Builder();
        builder.setLaunchSingleTop(obtainAttributes.getBoolean(C0428R.styleable.NavAction_launchSingleTop, false));
        builder.setPopUpTo(obtainAttributes.getResourceId(C0428R.styleable.NavAction_popUpTo, -1), obtainAttributes.getBoolean(C0428R.styleable.NavAction_popUpToInclusive, false));
        builder.setEnterAnim(obtainAttributes.getResourceId(C0428R.styleable.NavAction_enterAnim, -1));
        builder.setExitAnim(obtainAttributes.getResourceId(C0428R.styleable.NavAction_exitAnim, -1));
        builder.setPopEnterAnim(obtainAttributes.getResourceId(C0428R.styleable.NavAction_popEnterAnim, -1));
        builder.setPopExitAnim(obtainAttributes.getResourceId(C0428R.styleable.NavAction_popExitAnim, -1));
