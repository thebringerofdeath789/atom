...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        this.f60mX = new float[min];
        this.f61mY = new float[min];
        ...
        if (Math.abs(this.f60mX[0]) <= 1.0E-5d && Math.abs(this.f61mY[0]) <= 1.0E-5d) {
            int i3 = min - 1;
            if (Math.abs(this.f60mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f61mY[i3] - 1.0f) <= 1.0E-5d) {
                float f = 0.0f;
                int i4 = 0;
                while (i < min) {
                    float[] fArr2 = this.f60mX;
                    int i5 = i4 + 1;
                    float f2 = fArr2[i4];
                    if (f2 < f) {
        ...
        int i6 = min - 1;
        throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f60mX[0] + "," + this.f61mY[0] + " end:" + this.f60mX[i6] + "," + this.f61mY[i6]);
    }
