...
public class CameraParamTipController extends BaseController {
        ...
        this.wvEvItem.setOnWheelViewListener(new OnWheelScrolledListener() { // from class: com.ipotensic.kernel.controllers.CameraParamTipController.4
            ...
            public void onSelected(int i, String str) {
                ...
                try {
                    ...
                    DDLog.e(CameraParamTipController.TAG, "\u8bbe\u7f6eev\u503c\u8f6c\u6362\u6210double\u51fa\u9519", e);
                    ToastUtil.toast(CameraParamTipController.this.getContext(), CameraParamTipController.this.getContext().getString(R.string.dialog_failure));
                }
            }
        });
    ...
    private void findAllChildView() {
        this.rlFirst = (RelativeLayout) findView(R.id.rl_first);
        this.tvEvValue = (StrokeTextView) findView(R.id.tv_ev_value);
        this.tvSdValue = (StrokeTextView) findView(R.id.tv_sd_remain_time);
        this.tvResolution = (StrokeTextView) findView(R.id.tv_resolution_or_format);
        this.ivSd = (ImageView) findView(R.id.iv_sd);
        this.wvEvItem = (HorizontalWheelView) findView(R.id.wv_ev_item);
        this.ivSplitLineEv = (ImageView) findView(R.id.iv_split_line_ev);
        this.llWvEvItemWrapper = (LinearLayout) findView(R.id.ll_wv_et_item_wrapper);
        this.ivEvIcon = (ImageView) findView(R.id.iv_ev_icon);
        this.tvWhiteBalanceValue = (StrokeTextView) findView(R.id.tv_white_balance_value);
        this.tvSsValue = (StrokeTextView) findView(R.id.tv_ss_value);
        this.tvIsoValue = (StrokeTextView) findView(R.id.tv_iso_value);
        this.ivSplitSsWb = (ImageView) findView(R.id.iv_split_ss_wb);
        this.ivSplitIsoSs = (ImageView) findView(R.id.iv_split_iso_ss);
        this.ivHighTemp = (ImageView) findView(R.id.iv_high_temp);
        LinearLayout linearLayout = (LinearLayout) findView(R.id.ll_wv_resolution_item_wrapper);
        LinearLayout linearLayout2 = (LinearLayout) findView(R.id.ll_wv_fps_item_wrapper);
        HorizontalWheelView horizontalWheelView = (HorizontalWheelView) findView(R.id.wv_resolution_item);
        HorizontalWheelView horizontalWheelView2 = (HorizontalWheelView) findView(R.id.wv_fps_item);
        this.resolutionAndFpsSelectView = new ResolutionAndFpsSelectView(linearLayout, linearLayout2, (ImageView) findView(R.id.view_resolution_fps_split), this.ivSplitLineEv, horizontalWheelView, horizontalWheelView2);
    ...
    public void update() {
        String currentValue;
        DDLog.e("\u5f55\u50cf/\u62cd\u7167\u6a21\u5f0f1 = " + CameraConfig.get().getCaptureMode());
        if (CameraConfig.get().getCaptureMode() != null) {
            CaptureMode captureMode = CameraConfig.get().getCaptureMode();
            DDLog.e("\u5f55\u50cf/\u62cd\u7167\u6a21\u5f0f2 = " + captureMode);
            if (captureMode == CaptureMode.MODE_REC) {
                ...
                DDLog.e("\u5206\u8fa8\u7387 rec_res:" + currentValue2 + ", resolution:" + this.resolution);
                this.ev = CameraConfig.get().getRecordEV();
                DDLog.e("ev ev:" + this.ev);
                ...
                DDLog.e("\u5206\u8fa8\u7387 rec_res:" + currentValue + ", resolution:" + this.resolution);
                this.ev = CameraConfig.get().getPhotoEV();
                DDLog.e("ev ev1:" + this.ev);
            }
            DDLog.e("\u5f55\u50cf/\u62cd\u7167 \u5361\u72b6\u6001 = " + CameraConfig.get().sdCardState);
            if (CameraConfig.get().sdCardState == SdCardState.SD_CARD_NOT_EXIST) {
            ...
            DDLog.e("\u5237\u65b0\u53c2\u65701");
            setAllValidData();
            this.ivSd.setImageResource(R.mipmap.img_sd);
            this.ivEvIcon.setImageResource(R.mipmap.icon_er);
            return;
        }
        DDLog.e("\u5237\u65b0\u53c2\u65702");
        setAllNotApplicableData();
        this.ivSd.setImageResource(R.mipmap.img_sd_dis);
        this.ivEvIcon.setImageResource(R.mipmap.icon_er_dis);
    }
    ...
    public String getPhotoOrVideoCapacity() {
        ...
        if (CameraConfig.get().getCaptureMode() == CaptureMode.MODE_PHOTO) {
            DDLog.w("\u62cd\u7167 \u5269\u4f59\u62cd\u7167\u6570 = " + CameraConfig.get().getRecordRemain());
            return CameraConfig.get().getPhotoRemain() + " P";
        }
        DDLog.w("\u5f55\u50cf \u5269\u4f59\u5f55\u50cf\u65f6\u95f4 = " + CameraConfig.get().getRecordRemain());
    ...
    private void setTextGray(TextView textView) {
        textView.setTextColor(getContext().getColor(R.color.color_gray));
    }

    private void setAllValidData() {
        if (noViewClicked()) {
            DDLog.e("\u5237\u65b0\u53c2\u65703aaa\uff1a" + this.resolution);
            controlPhotoFormatView(this.isTimedMode);
            this.tvResolution.setText(this.resolution);
            if (!CameraConfig.get().isManualMode()) {
                setTextWhite(this.tvEvValue, getShowEvValue(this.ev));
            }
            setTextWhite(this.tvSdValue, this.sdSpaceSizes);
            return;
        }
        DDLog.e("\u5237\u65b0\u53c2\u65704aaa: " + this.resolution);
        this.tvResolution.setText(this.resolution);
        if (!CameraConfig.get().isManualMode()) {
            this.tvEvValue.setText(getShowEvValue(this.ev));
        }
    ...
    private int getGreenColor() {
        return getContext().getColor(R.color.text_color_camera_param_selected);
    }
    ...
    private abstract class OnValidClickListener implements View.OnClickListener {
        ...
        public void onClick(View view) {
            ...
            if (CameraConfig.get().getManualModeInfo() == null || !CameraConfig.get().getManualModeInfo().isManualMode) {
                CameraParamTipController.this.rlFirst.setBackgroundResource(R.drawable.bg_camera_param_tips);
                onValidClick(view);
                CameraParamTipController.this.showGrayBackgroundInFixedTime();
                return;
            }
            int id = view.getId();
            if (id == R.id.tv_ev_value || id == R.id.tv_sd_remain_time || id == R.id.tv_resolution_or_format) {
    ...
    private synchronized void show(int i) {
        ...
        if (i == -1) {
            ...
            if (this.showToastTag == 0) {
                this.showToastTag = 1;
                ToastUtil.toast(getContext(), getContext().getString(R.string.tips_cam_temperature_too_high));
            }
        } else if (i == 1) {
            if (this.isStart == null) {
                this.isStart = true;
            ...
            if (this.showToastTag < 2) {
                this.showToastTag = 2;
                ToastUtil.toast((Activity) getContext(), getContext().getString(R.string.tips_cam_stop_record), getContext().getResources().getColor(R.color.text_mini_geo_color));
            }
        } else if (i == 2) {
            if (this.isStart == null) {
                this.isStart = true;
            ...
            if (this.showToastTag < 3) {
                this.showToastTag = 3;
                ToastUtil.toast((Activity) getContext(), getContext().getString(R.string.tips_flight_auto_shut_down), getContext().getResources().getColor(R.color.toast_red));
            }
        }
    }
    ...
    public void onEvent(EventID eventID, Event event) {
        ...
        if (eventID == EventID.EVENT_CAMERA_HIGH_TEMP) {
            ...
            if (FlightRevData.get().getFlightRevStateData().isFlight()) {
                ...
                DDLog.e("\u76f8\u673a\u6e29\u5ea6\uff1ahighTempMark = " + this.highTempMark + ", isCaptureEnable = " + CameraConfig.get().isAllowCaptureInHighTemp() + ", \u6e29\u5ea6\uff1a" + event.arg2);
