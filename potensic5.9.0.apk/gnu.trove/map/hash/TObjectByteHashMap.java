...
public class TObjectByteHashMap<K> extends TObjectHash<K> implements TObjectByteMap<K>, Externalizable {
    ...
    public TObjectByteHashMap() {
        this.PUT_ALL_PROC = new TObjectByteProcedure<K>() { // from class: gnu.trove.map.hash.TObjectByteHashMap.1
            ...
            public boolean execute(K k, byte b) {
                TObjectByteHashMap.this.put(k, b);
                return true;
            }
        };
    ...
    public TObjectByteHashMap(int i, float f, byte b) {
        super(i, f);
        this.PUT_ALL_PROC = new TObjectByteProcedure<K>() { // from class: gnu.trove.map.hash.TObjectByteHashMap.1
            @Override // gnu.trove.procedure.TObjectByteProcedure
            public boolean execute(K k, byte b2) {
                TObjectByteHashMap.this.put(k, b2);
                return true;
            }
        };
    ...
    public TObjectByteHashMap(TObjectByteMap<? extends K> tObjectByteMap) {
        this(tObjectByteMap.size(), 0.5f, tObjectByteMap.getNoEntryValue());
        if (tObjectByteMap instanceof TObjectByteHashMap) {
            TObjectByteHashMap tObjectByteHashMap = (TObjectByteHashMap) tObjectByteMap;
            this._loadFactor = Math.abs(tObjectByteHashMap._loadFactor);
            byte b = tObjectByteHashMap.no_entry_value;
            this.no_entry_value = b;
            if (b != 0) {
                Arrays.fill(this._values, b);
            }
    ...
    protected class KeyView extends TObjectByteHashMap<K>.MapBackedView<K> {
        ...
        public boolean removeElement(K k) {
            return TObjectByteHashMap.this.no_entry_value != TObjectByteHashMap.this.remove(k);
        }
    ...
    class TByteValueCollection implements TByteCollection {
        ...
        @Override // gnu.trove.TByteCollection
        public boolean isEmpty() {
            return TObjectByteHashMap.this._size == 0;
        }

        @Override // gnu.trove.TByteCollection
        public boolean contains(byte b) {
            return TObjectByteHashMap.this.containsValue(b);
        }
        ...
        public boolean remove(byte b) {
            byte[] bArr = TObjectByteHashMap.this._values;
        ...
        public boolean containsAll(byte[] bArr) {
            for (byte b : bArr) {
                if (!TObjectByteHashMap.this.containsValue(b)) {
                    return false;
                }
            }
            return true;
        ...
        public boolean retainAll(byte[] bArr) {
            Arrays.sort(bArr);
            byte[] bArr2 = TObjectByteHashMap.this._values;
            Object[] objArr = TObjectByteHashMap.this._set;
            int length = objArr.length;
            boolean z = false;
            while (true) {
        ...
        public boolean forEach(TByteProcedure tByteProcedure) {
            return TObjectByteHashMap.this.forEachValue(tByteProcedure);
        }
        ...
        class TObjectByteValueHashIterator implements TByteIterator {
            ...
            TObjectByteValueHashIterator() {
                ...
                this._hash = tObjectByteHashMap;
                this._expectedSize = tObjectByteHashMap.size();
    ...
    class TObjectByteHashIterator<K> extends TObjectHashIterator<K> implements TObjectByteIterator<K> {
        ...
        public TObjectByteHashIterator(TObjectByteHashMap<K> tObjectByteHashMap) {
