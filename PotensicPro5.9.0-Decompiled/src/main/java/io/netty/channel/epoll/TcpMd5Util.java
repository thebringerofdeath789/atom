...
final class TcpMd5Util {
    static Collection<InetAddress> newTcpMd5Sigs(AbstractEpollChannel abstractEpollChannel, Collection<InetAddress> collection, Map<InetAddress, byte[]> map) throws IOException {
        ...
        for (Map.Entry<InetAddress, byte[]> entry : map.entrySet()) {
            ...
            if (value == null) {
                throw new NullPointerException("newKeys[" + entry.getKey() + PropertyUtils.INDEXED_DELIM2);
            }
            if (value.length == 0) {
                throw new IllegalArgumentException("newKeys[" + entry.getKey() + "] has an empty key.");
            }
            if (value.length > Native.TCP_MD5SIG_MAXKEYLEN) {
                throw new IllegalArgumentException("newKeys[" + entry.getKey() + "] has a key with invalid length; should not exceed the maximum length (" + Native.TCP_MD5SIG_MAXKEYLEN + PropertyUtils.MAPPED_DELIM2);
            }
        }
