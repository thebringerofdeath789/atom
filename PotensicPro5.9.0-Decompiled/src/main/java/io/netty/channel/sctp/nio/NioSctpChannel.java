...
public class NioSctpChannel extends AbstractNioMessageChannel implements SctpChannel {
    ...
    public Association association() {
        try {
            return mo46javaChannel().association();
    ...
    public Set<InetSocketAddress> allLocalAddresses() {
        try {
            Set allLocalAddresses = mo46javaChannel().getAllLocalAddresses();
    ...
    public Set<InetSocketAddress> allRemoteAddresses() {
        try {
            Set remoteAddresses = mo46javaChannel().getRemoteAddresses();
    ...
    public com.sun.nio.sctp.SctpChannel mo46javaChannel() {
        return super.mo46javaChannel();
    }

    @Override // io.netty.channel.Channel
    public boolean isActive() {
        return mo46javaChannel().isOpen() && association() != null;
    }

    @Override // io.netty.channel.AbstractChannel
    protected SocketAddress localAddress0() {
        try {
            Iterator it = mo46javaChannel().getAllLocalAddresses().iterator();
            if (it.hasNext()) {
    ...
    protected SocketAddress remoteAddress0() {
        try {
            Iterator it = mo46javaChannel().getRemoteAddresses().iterator();
            if (it.hasNext()) {
                return (SocketAddress) it.next();
            }
            return null;
    ...
    protected void doBind(SocketAddress socketAddress) throws Exception {
        mo46javaChannel().bind(socketAddress);
    }

    @Override // io.netty.channel.nio.AbstractNioChannel
    protected boolean doConnect(SocketAddress socketAddress, SocketAddress socketAddress2) throws Exception {
        if (socketAddress2 != null) {
            mo46javaChannel().bind(socketAddress2);
        }
        try {
            boolean connect = mo46javaChannel().connect(socketAddress);
            if (!connect) {
    ...
    protected int doReadMessages(List<Object> list) throws Exception {
        com.sun.nio.sctp.SctpChannel mo46javaChannel = mo46javaChannel();
        ...
        try {
            ...
            MessageInfo receive = mo46javaChannel.receive(internalNioBuffer, (Object) null, this.notificationHandler);
            if (receive != null) {
    ...
    protected boolean doWriteMessage(Object obj, ChannelOutboundBuffer channelOutboundBuffer) throws Exception {
        ...
        return mo46javaChannel().send(nioBuffer, createOutgoing) > 0;
    }
    ...
    public ChannelFuture bindAddress(final InetAddress inetAddress, final ChannelPromise channelPromise) {
        if (eventLoop().inEventLoop()) {
            try {
                mo46javaChannel().bindAddress(inetAddress);
    ...
    public ChannelFuture unbindAddress(final InetAddress inetAddress, final ChannelPromise channelPromise) {
        if (eventLoop().inEventLoop()) {
            try {
                mo46javaChannel().unbindAddress(inetAddress);
                channelPromise.setSuccess();
            } catch (Throwable th) {
                channelPromise.setFailure(th);
            }
