...
public class DefaultHttp2ConnectionEncoder implements Http2ConnectionEncoder {
    ...
    public ChannelFuture writeHeaders(ChannelHandlerContext channelHandlerContext, int i, Http2Headers http2Headers, int i2, short s, boolean z, int i3, boolean z2, ChannelPromise channelPromise) {
        ...
        try {
            ...
            if (stream == null) {
                ...
                int i4 = AnonymousClass2.$SwitchMap$io$netty$handler$codec$http2$Http2Stream$State[stream.state().ordinal()];
                if (i4 != 1 && i4 != 2) {
                    if (i4 == 3) {
                        stream.open(z2);
                    } else {
            ...
            if (cause == null) {
                ...
                this.lifecycleManager.onError(channelHandlerContext, cause);
            }
            return writeHeaders;
        } catch (Throwable th) {
            this.lifecycleManager.onError(channelHandlerContext, th);
    ...
    private final class FlowControlledHeaders extends FlowControlledBase {
        ...
        public void write(ChannelHandlerContext channelHandlerContext, int i) {
            ...
            Throwable cause = DefaultHttp2ConnectionEncoder.this.frameWriter.writeHeaders(channelHandlerContext, this.stream.id(), this.headers, this.streamDependency, this.weight, this.exclusive, this.padding, this.endOfStream, this.promise).cause();
            if (cause != null) {
                DefaultHttp2ConnectionEncoder.this.lifecycleManager.onError(channelHandlerContext, cause);
            } else {
