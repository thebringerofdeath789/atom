...
public abstract class AbstractByteBuf extends ByteBuf {
    ...
    final void ensureWritable0(int i) {
        ensureAccessible();
        if (i <= writableBytes()) {
            return;
        }
    ...
    public String toString() {
        ...
        StringBuilder append = new StringBuilder().append(StringUtil.simpleClassName(this)).append("(ridx: ").append(this.readerIndex).append(", widx: ").append(this.writerIndex).append(", cap: ").append(capacity());
        if (this.maxCapacity != Integer.MAX_VALUE) {
            append.append('/').append(this.maxCapacity);
        }
    ...
    protected final void checkNewCapacity(int i) {
        ...
        if (i < 0 || i > maxCapacity()) {
            throw new IllegalArgumentException("newCapacity: " + i + " (expected: 0-" + maxCapacity() + PropertyUtils.MAPPED_DELIM2);
        }
    }
