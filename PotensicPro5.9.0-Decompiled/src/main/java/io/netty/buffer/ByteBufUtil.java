...
public final class ByteBufUtil {
    ...
    public static int writeUtf8(ByteBuf byteBuf, CharSequence charSequence) {
        ...
        abstractByteBuf.ensureWritable0(utf8MaxBytes(charSequence));
    ...
    public static byte[] getBytes(ByteBuf byteBuf, int i, int i2, boolean z) {
        if (MathUtil.isOutOfBounds(i, i2, byteBuf.capacity())) {
            throw new IndexOutOfBoundsException("expected: 0 <= start(" + i + ") <= start + length(" + i2 + ") <= buf.capacity(" + byteBuf.capacity() + PropertyUtils.MAPPED_DELIM2);
        }
        if (byteBuf.hasArray()) {
    ...
    public static void copy(AsciiString asciiString, int i, ByteBuf byteBuf, int i2, int i3) {
        if (MathUtil.isOutOfBounds(i, i3, asciiString.length())) {
            throw new IndexOutOfBoundsException("expected: 0 <= srcIdx(" + i + ") <= srcIdx + length(" + i3 + ") <= srcLen(" + asciiString.length() + PropertyUtils.MAPPED_DELIM2);
        }
    ...
    public static void copy(AsciiString asciiString, int i, ByteBuf byteBuf, int i2) {
        if (MathUtil.isOutOfBounds(i, i2, asciiString.length())) {
            throw new IndexOutOfBoundsException("expected: 0 <= srcIdx(" + i + ") <= srcIdx + length(" + i2 + ") <= srcLen(" + asciiString.length() + PropertyUtils.MAPPED_DELIM2);
        }
    ...
    private static final class HexUtil {
        ...
        public static void appendPrettyHexDump(StringBuilder sb, ByteBuf byteBuf, int i, int i2) {
            if (MathUtil.isOutOfBounds(i, i2, byteBuf.capacity())) {
                throw new IndexOutOfBoundsException("expected: 0 <= offset(" + i + ") <= offset + length(" + i2 + ") <= buf.capacity(" + byteBuf.capacity() + PropertyUtils.MAPPED_DELIM2);
            }
            if (i2 == 0) {
                return;
            }
