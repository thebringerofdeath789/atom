...
public class CircleBatteryView extends View {
    ...
    private void init() {
        ...
        this.fillPaint.setColor(getContext().getResources().getColor(R.color.dialog_bg));
    ...
    private void drawFilledCircle(Canvas canvas) {
        int i = this.remainBattery;
        if (i >= 0 && i <= 20) {
            this.fillPaint.setColor(getContext().getResources().getColor(R.color.color_kernel_top_battery_light_red));
        } else {
            this.fillPaint.setColor(getContext().getResources().getColor(R.color.dialog_bg));
        }
        int i2 = this.width;
    ...
    private void drawProgressCircle(Canvas canvas) {
        ...
        int i2 = (i * 360) / 100;
        if (i == 100) {
            this.progressPaint.setColor(getContext().getResources().getColor(R.color.color_kernel_top_battery_green));
            canvas.drawArc(this.progressRect, 0.0f, 360.0f, false, this.progressPaint);
            return;
        }
        if (20 < i && i < 100) {
            this.progressPaint.setColor(getContext().getResources().getColor(R.color.color_gray));
        ...
        this.progressPaint.setColor(getContext().getResources().getColor(R.color.color_kernel_top_battery_deep_red));
        canvas.drawArc(this.progressRect, -90.0f, 360 - i2, false, this.progressPaint);
        this.progressPaint.setColor(getContext().getResources().getColor(R.color.color_kernel_top_battery_red));
        canvas.drawArc(this.progressRect, 270 - i2, i2, false, this.progressPaint);
    }
    ...
    private void drawProgressTxt(Canvas canvas) {
        int i = this.remainBattery;
        if (20 < i && i <= 100) {
            this.txtPaint.setColor(getContext().getResources().getColor(R.color.white));
        } else {
            this.txtPaint.setColor(getContext().getResources().getColor(R.color.white));
        }
    ...
    private void setDisableView(Canvas canvas) {
        this.progressPaint.setColor(getContext().getResources().getColor(R.color.colorWhite));
        canvas.drawArc(this.progressRect, 0.0f, 360.0f, false, this.progressPaint);
        int i = this.width;
        int i2 = this.height;
