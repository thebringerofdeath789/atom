...
public class TemperatureErrorDialog extends BaseSyncDialog {
    ...
    public TemperatureErrorDialog(Context context) {
        super(context, R.layout.view_dialog_temperature_error);
        this.mTemperature = Integer.MAX_VALUE;
        this.mErrorType = TemperatureErrorType.UNDEFINE;
        this.ivTemperatureError = (ImageView) findViewById(R.id.iv_battery_temperature_error);
        this.tvTemperature = (TextView) findViewById(R.id.tv_battery_temperature);
        this.tvTemperatureErrorNotice = (TextView) findViewById(R.id.tv_battery_temperature_error_notice);
    }
    ...
    private void updateTemperatureErrorType(TemperatureErrorType temperatureErrorType) {
        String format;
        this.ivTemperatureError.setImageResource(temperatureErrorType == TemperatureErrorType.HIGH ? R.mipmap.icon_high_temperature : R.mipmap.icon_low_temperature);
        if (temperatureErrorType == TemperatureErrorType.HIGH) {
            format = getContext().getString(R.string.battery_temperature_need_lower);
        } else {
            format = String.format(getContext().getString(R.string.battery_temperature_need_higher), 5);
        }
