...
public class GalleryActivity extends BaseActivity implements EventDispatcher.OnEventListener {
    ...
    public void download() {
        ...
        for (int i4 = 0; i4 < size; i4++) {
            if (this.list.get(i4).isSelected()) {
                i2++;
                if (this.list.get(i4).isDownloaded()) {
                    i3++;
                }
            }
        }
        if (i2 == i3 && i2 != 0) {
            if (i3 == 1) {
                i = R.string.dialog_file_is_downloaded;
            } else {
                i = R.string.dialog_files_are_downloaded;
            }
        ...
        if (i2 > 0) {
            DownloadFileDialog downloadFileDialog = new DownloadFileDialog(this, i2 - i3, new DownloadFileDialog.CancelListener() { // from class: com.ipotensic.kernel.activitys.GalleryActivity.13
            ...
            PhoneConfig.threadPool.execute(new Runnable() { // from class: com.ipotensic.kernel.activitys.GalleryActivity.14
                @Override // java.lang.Runnable
                public void run() {
                    for (int i5 = 0; i5 < size && !GalleryActivity.this.isDownloadCanceled; i5++) {
                        try {
                            RemoteFile remoteFile = (RemoteFile) GalleryActivity.this.list.get(i5);
    ...
    public void downloadFromUsb() {
        ...
        for (int i4 = 0; i4 < size; i4++) {
            if (this.list.get(i4).isSelected()) {
                ...
                if (this.list.get(i4).isDownloaded()) {
                    ...
                    this.localFiles.add(this.list.get(i4));
                }
            }
        }
        if (i2 <= 0) {
        ...
        this.isDownloadCanceled = false;
        if (i2 > 0) {
            DownloadFileDialog downloadFileDialog = new DownloadFileDialog(this, i2 - i3, new DownloadFileDialog.CancelListener() { // from class: com.ipotensic.kernel.activitys.GalleryActivity.15
                @Override // com.ipotensic.kernel.view.DownloadFileDialog.CancelListener
                public void onCancelClicked() {
                    GalleryActivity.this.showLoadingDialog();
            ...
            try {
                RemoteFileManager.getInstance().downloadMultiFileForUsb(this.localFiles, this.downloadDialog.getDownloadListener(), new OnFileDownloadListener() { // from class: com.ipotensic.kernel.activitys.GalleryActivity.16
                    ...
                    public void onItemDownloaded(int i5) {
                    }
