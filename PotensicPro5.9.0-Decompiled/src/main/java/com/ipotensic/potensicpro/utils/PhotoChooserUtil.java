...
public class PhotoChooserUtil {
    ...
    private void requestRuntimePermission(final int i, String[] strArr, PermissionListener permissionListener) {
        ...
        String string = this.context.getString(R.string.button_refuse);
        String string2 = this.context.getString(R.string.button_allow);
        if (!SPHelper.getInstance().getBoolean("key_is_first_camera_and_storage_permission_show", false)) {
            ...
            string = this.context.getString(R.string.dialog_cancel);
            string2 = this.context.getString(R.string.dialog_location_confirm);
        }
        AppCompatActivity appCompatActivity = this.context;
        String string3 = appCompatActivity.getString(R.string.feedback_or_avatar_upload_camera_and_photos_access_title);
        String string4 = this.context.getString(R.string.feedback_or_avatar_upload_camera_and_photos_access_tips);
            ...
            public void run() {
                ...
                try {
                    ...
                    if (DocumentsContract.isDocumentUri(PhotoChooserUtil.this.context, uri)) {
                        ...
                        DDLog.e("反馈", "图片authority： " + authority + ", docId = " + documentId);
        ...
        new Thread(new Runnable() { // from class: com.ipotensic.potensicpro.utils.PhotoChooserUtil.9
            ...
            public void run() {
                ...
                if (DocumentsContract.isDocumentUri(PhotoChooserUtil.this.context, uri)) {
                    String authority = uri.getAuthority();
                    String documentId = DocumentsContract.getDocumentId(uri);
                    DDLog.e("反馈", "authority： " + authority + ", docId = " + documentId);
                    if ("com.android.providers.media.documents".equals(authority)) {
                        String str3 = "_id=" + documentId.split(":")[1];
                        DDLog.e("-------打开图库selection---------: " + str3);
