...
public class DroneGuideActivity extends BaseActivity {
    ...
    private void setToolBar() {
        ...
        toolbar.findViewById(R.id.iv_back).setOnClickListener(new View.OnClickListener() { // from class: com.ipotensic.potensicpro.activities.DroneGuideActivity.1
    ...
    private void setText() {
        String string;
        try {
            String string2 = getString(R.string.me_find_my_drone_title);
            if (FlightConfig.is_Atom_SE_Series()) {
                string = getString(R.string.me_find_my_drone_atom_se_tips);
            } else if (FlightConfig.isAtomLT()) {
                string = getString(R.string.me_find_my_drone_atom_lt_tips);
            } else {
                string = getString(R.string.me_find_my_drone_atom_tips);
            }
            this.tvContent.setText(string.substring(string2.length() + 1, string.length()));
        } catch (Exception e) {
            DDLog.e("droneGuideSetText报错:" + e.getMessage());
        }
    }
}
