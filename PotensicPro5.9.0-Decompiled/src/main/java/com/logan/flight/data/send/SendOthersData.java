...
public class SendOthersData {
    ...
    public static byte[] getRemoterInfoBytes() {
        byte[] wrap = UsbPayloadWrapper.wrap(EscherAggregate.ST_FLOWCHARTPREDEFINEDPROCESS, new byte[]{FlightConfig.P1_PRO_RC_2, FlightConfig.P3_SE_V0});
        wrap[1] = -2;
        return wrap;
    }
    ...
    public static byte[] getOpenRemoterCalibrationBytes() {
        byte[] wrap = UsbPayloadWrapper.wrap(EscherAggregate.ST_FLOWCHARTINTERNALSTORAGE, new byte[]{FlightConfig.P1_SELF});
        wrap[1] = -2;
        return wrap;
    }

    public static byte[] getCloseRemoterCalibrationBytes() {
        byte[] wrap = UsbPayloadWrapper.wrap(EscherAggregate.ST_FLOWCHARTINTERNALSTORAGE, new byte[]{-16});
        wrap[1] = -2;
        return wrap;
    }

    public static byte[] getRemoterRockerDataBytes() {
        byte[] wrap = UsbPayloadWrapper.wrap(EscherAggregate.ST_FLOWCHARTDOCUMENT, new byte[]{0});
        wrap[1] = -2;
        return wrap;
    }

    public static byte[] getOpenRemoterMuteBytes() {
        byte[] wrap = UsbPayloadWrapper.wrap(EscherAggregate.ST_FLOWCHARTMULTIDOCUMENT, new byte[]{1});
        wrap[1] = -2;
        return wrap;
    }

    public static byte[] getCloseRemoterMuteBytes() {
        byte[] wrap = UsbPayloadWrapper.wrap(EscherAggregate.ST_FLOWCHARTMULTIDOCUMENT, new byte[]{0});
        wrap[1] = -2;
        return wrap;
    }
}
