...
public class BigPackageHelper {
    ...
    public void uploadTotalFile() {
        if (this.uploadFile != null) {
            try {
                ...
                getNextResult();
                DDLog.e("大包文件上传完成 ：" + this.offset);
            } catch (Exception e) {
                DDLog.e("大包上传文件异常:" + e.getMessage());
            }
        }
    }
        ...
        PhoneConfig.threadPool.execute(new Runnable() { // from class: com.logan.upgrade.big.BigPackageHelper.1
            ...
            public void run() {
                try {
                    ...
                    DDLog.e("大包文件上传 :" + BigPackageHelper.this.readLen);
                    if (BigPackageHelper.this.readLen != -1 && !BigPackageHelper.this.stopSendData) {
                        if (BigPackageHelper.this.offset != BigPackageHelper.this.needOffset) {
                            DDLog.e("大包文件上传 offset=" + BigPackageHelper.this.offset + ", needOffset=" + BigPackageHelper.this.needOffset);
                        ...
                        if (BigPackageHelper.this.readLen != BigPackageHelper.this.data.length) {
                            DDLog.e("大包最后一包文件上传：" + BigPackageHelper.this.readLen);
                        ...
                        BigPackageHelper.this.offset += BigPackageHelper.this.readLen;
                        DDLog.e("大包上传的当前偏移量offset :" + BigPackageHelper.this.offset);
                        PhoneConfig.mainHandler.postDelayed(BigPackageHelper.this.uploadTimeOutRunnable, 1000L);
                        DDLog.e("大包========================================================");
                    }
                } catch (Exception e) {
                    DDLog.e("大包 getNextResult exception " + e.getMessage());
                }
            }
        });
    }
    ...
    public void lambda$onReupload$1$BigPackageHelper() {
        int i = this.reUploadTotalTimes;
        if (i > 0) {
            this.reUploadTotalTimes = i - 1;
        } else if (i <= 0) {
            DDLog.e("大包 固件传输超时，退出升级");
        ...
        DDLog.e("大包 reUploadTotalTimes： " + this.reUploadTotalTimes);
        if (this.reUploadTimes >= 30) {
            DDLog.e("大包 单包固件传输超时，退出升级");
            this.reUploadTimes = 0;
            this.stopSendData = true;
            this.needReUpload = false;
            PhoneConfig.mainHandler.removeCallbacks(this.uploadTimeOutRunnable);
        ...
        if (this.isTimeout) {
            ...
            DDLog.e("大包超时重传offset：" + this.offset);
        } else {
            try {
            ...
            DDLog.e("大包重传offset：" + this.offset);
        }
        this.reUploadTimes++;
        getNextResult();
    }
    ...
    public void onReceivedUploadFile(byte[] bArr, int i) {
        DDLog.e("收到大包——文件上传:" + ParseUtil.byteToHexString(bArr, 20));
        byte b = bArr[i];
        byte b2 = bArr[i + 2];
        DDLog.e("收到大包cmd：" + ((int) b2));
    ...
    public void onReceivedUpgradeProgress(byte[] bArr, int i) {
        ...
        if (!this.isStartCheck) {
            DDLog.e("大包  开始检查相机下行数据");
            this.isStartCheck = true;
            checkReceivedUpgradeData();
        }
        DDLog.e("收到大包升级阶段：" + ParseUtil.byteToHexString(bArr));
        int i2 = i + 5;
        if (bArr.length > i2) {
            byte b = bArr[i + 2];
            if (b == 0) {
                DDLog.e("收到大包——升级进度0x00");
                ...
                DDLog.e("收到大包——升级进度：state: " + byteToDecimalInt + ", 总升级进度：" + byteToDecimalInt3);
            ...
            DDLog.e("收到大包——升级结果0x01, upgradeStage = " + byteToDecimalInt4 + ", errorCode = " + byteToDecimalInt5 + ", endUpgrade = " + z);
            deInit();
    ...
    private void checkReceivedUpgradeData() {
        if (this.receiveUpgradeDataRunnable == null) {
            CancelRunnable cancelRunnable = new CancelRunnable() { // from class: com.logan.upgrade.big.BigPackageHelper.2
                ...
                public void run() {
                    DDLog.e("大包  startTime= " + BigPackageHelper.this.startTime + ", System.currentTimeMillis()= " + System.currentTimeMillis() + ", 时间差= " + (System.currentTimeMillis() - BigPackageHelper.this.startTime));
                    if (System.currentTimeMillis() - BigPackageHelper.this.startTime >= 60000) {
                        DDLog.e("大包 超过60s没有接收到相机下发的数据");
    ...
    public void onReceivedError(byte b, int i, byte[] bArr) {
        DDLog.e("收到大包上传文件失败消息resultCode = " + ((int) b));
        DDLog.e("收到大包上传文件失败消息data = " + ParseUtil.byteToHexString(bArr));
        if (this.stopSendData) {
            return;
        }
        int i2 = i + 2;
        if (bArr[i2] != 1) {
            ...
            synchronized (this.upgradeLock) {
                if (!this.stopSendData) {
                    ...
                    PhoneConfig.mainHandler.removeCallbacks(this.uploadTimeOutRunnable);
                    this.needOffset = ParseUtil.byteArrayToLong(bArr, i + 3);
                    DDLog.e("收到大包文件上传期待的偏移量:" + this.needOffset);
                }
                onReupload();
            }
        }
