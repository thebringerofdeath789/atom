...
public class AOAEngine {
    ...
    private final BroadcastReceiver usbAttachedReceiver = new BroadcastReceiver() { // from class: com.logan.usb.AOAEngine.3
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            DDLog.e("DDLog-AOAENGINE", "onReceive: " + action);
            if ("android.hardware.usb.action.USB_ACCESSORY_DETACHED".equals(intent.getAction())) {
                DDLog.e("DDLog-AOAENGINE", "收到广播:断开 " + action);
                AOAEngine.this.Quit.set(true);
                return;
            }
            if ("android.hardware.usb.action.USB_ACCESSORY_ATTACHED".equals(intent.getAction())) {
                DDLog.e("DDLog-AOAENGINE", "收到广播:连接 " + action);
                return;
            }
            if ("android.hardware.usb.action.USB_STATE".equals(intent.getAction())) {
                if (intent.getExtras().getBoolean("connected")) {
                    DDLog.e("DDLog-AOAENGINE", "收到广播:插入0 " + action);
                    DDLog.e("DDLog-AOAENGINE", "收到广播:插入1 " + AOAEngine.this.connectRunnable);
                ...
                DDLog.e("DDLog-AOAENGINE", "收到广播:拔出 " + action);
                AOAEngine.this.Quit.set(true);
                if (AOAEngine.this.connectRunnable != null) {
                    AOAEngine.this.handler.removeCallbacks(AOAEngine.this.connectRunnable);
                    AOAEngine.this.connectRunnable = null;
                    DDLog.e("DDLog-AOAENGINE", "清空 runnable ");
                }
            }
        }
    };
    private final BroadcastReceiver mPermissionReceiver = new BroadcastReceiver() { // from class: com.logan.usb.AOAEngine.4
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            DDLog.d("DDLog-AOAENGINE", "接收权限同意广播: ");
            if (intent.getBooleanExtra("permission", false)) {
                AOAEngine.this.connect();
            }
        }
    };
        ...
        public void run() {
            ...
            try {
                ...
                if (AOAEngine.this.parcelFileDescriptor == null) {
                    DDLog.e("DDLog-AOAENGINE", "不能打开AOA");
                ...
                try {
                    ...
                    DDLog.e("send test data:" + e2.getMessage());
                    AOAEngine.this.Quit.set(true);
                    z = false;
                }
                if (z) {
                ...
                while (!AOAEngine.this.Quit.get()) {
                    try {
                        ...
                        DDLog.e("DDLog-AOAENGINE", "遥控器通道断开：" + e4.getMessage());
                    }
                }
                AOAEngine.this.Quit.set(true);
                AOAEngine.this.sendBytes.clear();
                ...
                DDLog.e("DDLog-AOAENGINE", "usb读取结束");
                if (AOAEngine.this.h264Decoder != null) {
                    AOAEngine.this.h264Decoder.release(null);
                    AOAEngine.this.h264Decoder = null;
                }
                DDLog.d("DDLog-AOAENGINE", "exiting reader thread");
                if (AOAEngine.this.parcelFileDescriptor != null) {
                    try {
                        AOAEngine.this.parcelFileDescriptor.close();
                    } catch (Exception unused) {
                        DDLog.e("DDLog-AOAENGINE", "Unable to close ParcelFD");
                    }
                }
                if (AOAEngine.this.inputStream != null) {
                    try {
                        ...
                        DDLog.e("DDLog-AOAENGINE", "Unable to close InputStream");
                    }
                }
                if (AOAEngine.this.outputStream != null) {
                    try {
                        ...
                        DDLog.e("DDLog-AOAENGINE", "Unable to close OutputStream");
                    }
                }
                AOAEngine.this.isAccessoryConnected = false;
                AOAEngine.this.Quit.set(true);
                AOAEngine.this.accessoryThread = null;
                DDLog.e("DDLog-AOAENGINE", "释放完成");
                if (z) {
    ...
    private AOAEngine() {
        DDLog.e("DDLog-AOAENGINE", "创建AOAEngine");
    }
    ...
    public synchronized void start() {
        ...
        if (accessoryList != null) {
            ...
            DDLog.e("DDLog-AOAENGINE", "找到AOA设备start mAccessory: " + this.Accessory);
            connect();
        }
    }
    ...
    public synchronized void connect() {
        ...
        if (this.Accessory != null) {
            DDLog.e("DDLog-AOAENGINE", "是否有权限:" + this.usbManager.hasPermission(this.Accessory));
            if (!this.usbManager.hasPermission(this.Accessory)) {
                return;
            }
            DDLog.e("DDLog-AOAENGINE", "connect: 开启通道。。。");
            if (this.accessoryThread == null) {
                ...
                DDLog.d("DDLog-AOAENGINE", "reader thread already started");
            }
        } else {
            DDLog.d("DDLog-AOAENGINE", "accessory is null");
        }
    }
    ...
    public synchronized void send(byte[] bArr, byte b) {
        if (!this.Quit.get() && this.isAccessoryConnected && this.outputStream != null) {
            ...
            if (b == UsbConfig.USB_TYPE_APP_TO_CAMERA) {
                DDLog.e("发送相机数据：" + ParseUtil.byteToHexString(wrap, 50));
            } else if (b == UsbConfig.USB_TYPE_APP_TO_FLIGHT) {
                DDLog.e("发送飞控数据：" + ParseUtil.byteToHexString(wrap, 20));
            } else if (b == UsbConfig.USB_TYPE_APP_TO_REMOTER) {
                DDLog.e("发送遥控器数据：" + ParseUtil.byteToHexString(wrap, 20));
            }
    ...
    public static void output(int i, byte[] bArr) {
        DDLog.e("输出数据 数据类型 ：" + i + " , size : " + bArr.length + " , data : " + ParseUtil.byteToHexString(bArr, 10));
    }
