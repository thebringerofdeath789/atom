...
public class UsbCameraHandler implements IRequestClient {
    ...
    public void executeShortVideo(VisionExecuteType visionExecuteType, int i, int i2) {
        ...
        DDLog.e("发送执行或退出 一键短片指令:" + ParseUtil.byteToHexString(bArr));
        send(bArr);
    }
    ...
    public void setCameraTime() {
        ...
        DDLog.e("设置相机时间:" + ParseUtil.byteToHexString(bArr));
        send(bArr);
    }
    ...
    public void setTakePhotoMode(PhotoChildMode photoChildMode) {
        if (photoChildMode != null) {
            ...
            send(bArr);
            DDLog.e("设置拍照子模式：" + photoChildMode.toString());
        }
    }
            ...
            if (UsbConfig.getMsgId(bArr, 0) == 32) {
                if (bArr[i] == 57) {
                    try {
                        ...
                        DDLog.e("收到相机log:" + cameraLogData.toString());
                        return;
                    } catch (Exception e) {
                        DDLog.e("接收相机log错误 ：" + e);
                        return;
                ...
                if (bArr[i3] != 0) {
                    DDLog.e("相机数据 发生错误 ：" + ParseUtil.byteToHexString(bArr, 20));
                ...
                if (b == -14) {
                    ...
                    if (hardwareStateUploadData.code != 0) {
                        DDLog.e("相机硬件状态上报 " + hardwareStateUploadData);
                    }
                ...
                if (b == 11) {
                    byte b2 = bArr[i2];
                    DDLog.e("设置录像分辨率成功 index=" + ((int) b2));
                ...
                if (b == 40) {
                    DDLog.e("获取剩余拍照张数/录像时长：成功");
                    parseRemainCapture(ParseUtil.interceptBytes(bArr, i2, 3));
                    return;
                }
                if (b == 68) {
                    DDLog.e("收到报告大包升级握手状态：" + ParseUtil.byteToHexString(bArr));
                    DDLog.e("大包升级握手状态 data length：" + bArr.length + ", index：" + i);
                    int i6 = i + 3;
                    if (bArr.length > i6) {
                        String trim = new String(bArr, i2, bArr.length - i6, StandardCharsets.US_ASCII).trim();
                        DDLog.e("大包升级握手状态 json: " + trim);
                ...
                switch (b) {
                    ...
                    case 17:
                        ...
                        DDLog.e("config menu:" + CameraConfig.get().toString());
                        return;
                    default:
                        switch (b) {
                            case 22:
                            ...
                            case 35:
                                ...
                                if (bArr.length >= i8) {
                                    ...
                                    DDLog.e("收到相机高温数据:" + ParseUtil.byteToHexString(bArr));
                                    ...
                                    case 44:
                                        try {
                                            ...
                                            if (parseTargetTrack != null) {
                                                DDLog.e("收到视觉数据:" + parseTargetTrack.isSelected());
                                                EventDispatcher.get().sendEvent(EventID.EVENT_VISUAL_TARGET_TRACK_RESULT, parseTargetTrack);
                                                return;
                                            }
                                            return;
                                        } catch (Exception e3) {
                                            DDLog.e("视觉目标截取报错:" + e3.getMessage());
                                            return;
                                        }
                                    case 45:
                                        DDLog.e("收到视觉数据 是否跟踪到目标:" + (bArr[i2] == 1) + " ,  " + ParseUtil.byteToHexString(bArr));
                                        EventDispatcher.get().sendEvent(EventID.EVENT_VISUAL_TARGET_TRACKING);
                                        return;
                                    case 46:
                                        DDLog.e("收到视觉数据 是否执行一键短片:" + ParseUtil.byteToHexString(bArr));
                                        byte b8 = bArr[i3];
                                        if (bArr[i2] == 0) {
                                            DDLog.e("收到视觉数据 是否执行一键短片:退出");
                                            EventDispatcher.get().sendEvent(EventID.EVENT_VISUAL_TARGET_EXIT);
                                            return;
                                        } else {
                                            DDLog.e("收到视觉数据 是否执行一键短片:执行");
                                    ...
                                    default:
                                        switch (b) {
                                            ...
                                            case 49:
                                                ...
                                                break;
                                            case 50:
                                                DDLog.e("大包版本号：" + ParseUtil.byteToHexString(bArr));
                                                DDLog.e("大包 index：" + i);
                                                int i9 = i + 27;
                                                if (bArr.length > i9) {
                                                    ...
                                                    DDLog.e("大包 版本号product: " + trim2 + ", version: " + trim3 + ", 升级介质：" + byteToDecimalInt2);
                                            ...
                                            default:
                                                switch (b) {
                                                    case 52:
                                                        ...
                                                        DDLog.e(TAG, manualModeInfo.toString());
                                                    ...
                                                    case 54:
                                                        ...
                                                        DDLog.e(TAG, "自动模式曝光程度: " + ((int) b10) + " 色温值：" + unsignedShortFromByteArr);
                                                    ...
                                                    default:
                                                        switch (b) {
                                                            ...
                                                            case 61:
                                                                ...
                                                                DDLog.e("设置拍照子模式：成功 " + photoChildMode.toString());
                                                            ...
                                                            case 62:
                                                                ...
                                                                DDLog.e("设置数码变焦：成功,变焦倍数：" + intFromByteArr);
                                                                break;
                                                            case 63:
                                                            ...
                                                            case 65:
                                                                ...
                                                                DDLog.e("拍照子模式上报 " + timedPhotoUploadData.toString());
            ...
            if (b != 24) {
                if (b == 58) {
                    ...
                    switch (b) {
                        case 44:
                        case 46:
                            DDLog.e("收到视觉数据 错误1:" + ParseUtil.byteToHexString(bArr));
                            return;
                        case 45:
                            DDLog.e("收到视觉错误1:" + ParseUtil.byteToHexString(bArr));
                            EventDispatcher.get().sendEvent(EventID.EVENT_VISUAL_TARGET_ERROR, VisionError.ERROR_CAMERA_UNKNOWN);
                            break;
                        case 47:
                            DDLog.e("收到视觉数据 错误:" + ParseUtil.byteToHexString(bArr));
        ...
        if (b2 != 22) {
            if (b2 != 37) {
                if (b2 != 53) {
                    switch (b2) {
                        case 1:
                            DDLog.e("命令不支持:" + ParseUtil.byteToHexString(bArr));
                            EventDispatcher.get().sendEvent(EventID.EVENT_CMD_NOT_SUPPORT);
                            break;
                        case 2:
                            DDLog.e("参数无效:" + ParseUtil.byteToHexString(bArr));
                        ...
                        case 3:
                            ...
                            break;
                        case 4:
                            DDLog.e("未知错误:" + ParseUtil.byteToHexString(bArr));
                        ...
                        case 5:
                            ...
                            break;
                        case 6:
                            DDLog.e("相机 sd卡已满 ：" + ParseUtil.byteToHexString(bArr));
                        ...
                        case 7:
                            ...
                            break;
                        case 8:
                            DDLog.e("当前模式不允许:" + ParseUtil.byteToHexString(bArr));
                            break;
                        case 10:
    ...
    private CameraInfoData parseCameraState(byte[] bArr, int i) {
        ...
        DDLog.e("录像/拍照信息:" + cameraInfoData.toString());
        return cameraInfoData;
    }

    private void parseCardState(byte b, boolean z) {
        DDLog.e("相机 sd card state：" + ((int) b));
        CameraConfig.get().setSdState(b);
        DDLog.w("相机sd卡状态是否正常：" + CameraConfig.get().sdCardState);
        if (b != 8) {
            this.isSpeedNotStable = false;
        }
        switch (b) {
            case 0:
                DDLog.e("未检测到SD卡 11111111111");
                break;
            case 1:
                if (z) {
            ...
            case 4:
                ...
                break;
            case 5:
                DDLog.e("相机 sd卡已满1 ：" + ParseUtil.byteToHexString(new byte[]{b}));
                EventDispatcher.get().sendEvent(EventID.EVENT_SD_CARD_FULL);
                EventDispatcher.get().sendEvent(EventID.EVENT_SD_CARD_FULL_TIP);
                break;
            case 6:
    ...
    private void parseCameraInfo(byte[] bArr) {
        ...
        DDLog.e("相机信息:" + trim);
        DDLog.e("相机信息:" + ParseUtil.byteToHexString(bArr));
        ...
        if (upperCase.contains("V")) {
            ...
            DDLog.e("相机版本：" + substring);
            DDLog.e("相机型号：" + trim.substring(0, lastIndexOf));
    ...
    private void parseAllParams(byte[] bArr, int i) {
        DDLog.e("收到config menu 数据:" + ParseUtil.byteToHexString(bArr));
        ...
        CameraConfig.get().setSdFreeSpace(ParseUtil.getIntFromByteArr(bArr3, 0));
        DDLog.e("获取相机卡可用大小：" + CameraConfig.get().getSdFreeSpace());
        int i8 = i7 + 1;
        bArr3[0] = bArr[i8];
        int i9 = i8 + 1;
        bArr3[1] = bArr[i9];
        ...
        bArr3[3] = 0;
        CameraConfig.get().setSdTotalSpace(ParseUtil.getIntFromByteArr(bArr3, 0));
        DDLog.e("获取相机卡总共大小：" + CameraConfig.get().getSdTotalSpace());
        int i11 = i10 + 1;
        byte b = bArr[i11 + 1];
        ...
        for (int i15 = 0; i15 < CameraConfig.supportRecordEvValues.length; i15++) {
            DDLog.e("录制ev list:" + i15 + " , " + CameraConfig.supportRecordEvValues[i15]);
        }
        for (int i16 = 0; i16 < CameraConfig.supportTakePhotoEvValues.length; i16++) {
            DDLog.e("拍照ev list:" + i16 + " , " + CameraConfig.supportTakePhotoEvValues[i16]);
        }
        DDLog.e("录像 拍照ev:" + CameraConfig.get().getRecordEV());
        DDLog.e("录像 拍照ev:" + CameraConfig.get().getPhotoEV());
        int i17 = i14 + b4 + 1 + 1;
        byte b5 = bArr[i17 + 1];
        ...
        if (bArr.length >= i17 + b5 + 1 + 6 + 1) {
            ...
            DDLog.e("configmenu 拍照格式:" + CameraConfig.get().isRaw());
            DDLog.e("configmenu 录像水印:" + CameraConfig.get().isVideoOsdShow());
            DDLog.e("configmenu 拍照水印:" + CameraConfig.get().isPhotoOsdShow());
            int i21 = i20 + 1;
            parseRemainCapture(ParseUtil.interceptBytes(bArr, i21, 3));
            i17 = i21 + 3;
        }
        int i22 = i17 + 1;
        if (bArr.length > i22 + 6) {
            int i23 = bArr[i17] * 2;
        ...
        DDLog.e("getConfigMenu callback:");
        EventDispatcher.get().sendEvent(EventID.EVENT_GET_SD_CARD_STATE_SUCCESS);
    }
    ...
    private void parseRemainCapture(byte[] bArr) {
        ...
        DDLog.e("configmenu 剩余拍照张数/录像时间:" + intFromByteArr);
    }
    ...
    public void sendGalleryData(byte[] bArr) {
        ...
        DDLog.w("发送图库数据:" + ParseUtil.byteToHexString(wrap));
