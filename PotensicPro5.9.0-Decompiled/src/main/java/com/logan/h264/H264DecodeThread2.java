...
public class H264DecodeThread2 extends IDecoder {
    ...
    OnYuvCallback callback = new OnYuvCallback() { // from class: com.logan.h264.H264DecodeThread2.1
        ...
        public void callback(I420 i420) {
            if (i420 != null) {
                H264Player.onYuvCallback(i420.width, i420.height, i420.Y, i420.U, i420.V);
            }
        }
    };
        ...
        if (this.isStart) {
            try {
                ...
                if (this.frameNal.size() > 20) {
                    DDLog.e(this.TAG, "clear frameNal");
                    this.frameNal.clear();
                }
            } catch (Exception e) {
                DDLog.e(this.TAG, "h264解析出错:" + e.getMessage());
            }
        }
    }
        ...
        while (this.isStart) {
            while (this.isStart && this.frameNal.size() > 0) {
                try {
                    ...
                    DDLog.e("frame:" + ParseUtil.byteToHexString(data, 50));
                    ...
                    if (this.isInit) {
                        ...
                        if (System.currentTimeMillis() - this.timeStart > 1000) {
                            ...
                            if (this.times.size() != 0) {
                                DDLog.e(this.TAG, "解码耗时 fps:" + this.times.size());
                                DDLog.e(this.TAG, "解码耗时:" + (j / this.times.size()));
                            }
                            this.times.clear();
                    ...
                    DDLog.e(this.TAG, "解码报错:" + e2.getMessage());
                }
            }
            Thread.sleep(1L);
        }
        DDLog.e(this.TAG, "解码结束");
        this.frameNal.clear();
        h264ToYuvDecoder2.release();
