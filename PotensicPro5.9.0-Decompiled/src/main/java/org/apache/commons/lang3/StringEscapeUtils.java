...
public class StringEscapeUtils {
    ...
    public static final CharSequenceTranslator ESCAPE_ECMASCRIPT = new AggregateTranslator(new LookupTranslator(new String[]{"'", "\\'"}, new String[]{"\"", "\\\""}, new String[]{"\\", "\\\\"}, new String[]{InternalZipConstants.ZIP_FILE_SEPARATOR, "\\/"}), new LookupTranslator(EntityArrays.JAVA_CTRL_CHARS_ESCAPE()), JavaUnicodeEscaper.outsideOf(32, 127));
    public static final CharSequenceTranslator ESCAPE_JSON = new AggregateTranslator(new LookupTranslator(new String[]{"\"", "\\\""}, new String[]{"\\", "\\\\"}, new String[]{InternalZipConstants.ZIP_FILE_SEPARATOR, "\\/"}), new LookupTranslator(EntityArrays.JAVA_CTRL_CHARS_ESCAPE()), JavaUnicodeEscaper.outsideOf(32, 127));
    ...
    static class CsvUnescaper extends CharSequenceTranslator {
        ...
        public int translate(CharSequence charSequence, int i, Writer writer) throws IOException {
            ...
            if (StringUtils.containsAny(charSequence2, CSV_SEARCH_CHARS)) {
                ...
                String str = CSV_QUOTE_STR;
                writer.write(StringUtils.replace(charSequence2, sb.append(str).append(str).toString(), str));
            } else {
                writer.write(charSequence.toString());
            }
            return Character.codePointCount(charSequence, 0, charSequence.length());
